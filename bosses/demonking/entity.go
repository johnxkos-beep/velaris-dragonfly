...
const (
	phaseHealth       = 150.0 // +50% per phase, per request (was doubled to 200, before that 100 matching the add-on exactly)
	meleeDamage       = 9.0
	meleeRange        = 2.5
...
// isTargetable excludes creative/spectator players — they can't be hurt by
// most damage sources anyway (creative) or shouldn't be interactable at all
// (spectator), so there's no reason for him to chase or attack them.
...
func (e *DemonKing) snapToGround(tx *world.Tx, pos mgl64.Vec3) mgl64.Vec3 {
	...
	// Nothing solid found even 64 blocks down — hold his current height
	// instead of endlessly sinking. The old behaviour (sinking 0.3/tick
	// forever when no ground was found within just 10 blocks) is almost
	// certainly why he'd sometimes vanish shortly after spawning: he'd
	// sink into the void, where servers typically auto-remove entities.
	return pos
}
...
// phase transitions.
// dropTrophy spawns a single Trophy item at his position when he dies for
// good. Uses the same item-drop pattern Dragonfly's own Player.Drop uses
// internally (entity.NewItem + tx.AddEntity) — confirmed against real
// Dragonfly source, so reasonably high confidence this one's correct.
func (e *DemonKing) dropTrophy(tx *world.Tx) {
	stack := item.NewStack(Trophy{}, 1)
	handle := dfentity.NewItem(stack, e.data.Pos)
	tx.AddEntity(handle)
}

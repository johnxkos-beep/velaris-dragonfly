...
func (e *DemonKing) Hurt(dmg float64, src world.DamageSource) (float64, bool) {
	...
	// TEMPORARY DEBUG: tells you directly, in chat, whether Hurt is being
	// called at all and what his HP is. Once damage is confirmed working,
	// tell me and I'll strip this out.
	if s, ok := src.(dfentity.AttackDamageSource); ok {
		if p, ok := s.Attacker.(*player.Player); ok {
			p.Message("Demon King hit for ", dmg, " — HP: ", e.fight.HP, "/", e.fight.MaxHP, " (phase ", e.fight.Phase, ")")
		}
	}
...
func (e *DemonKing) Tick(tx *world.Tx, current int64) {
	...
	// TEMPORARY DEBUG: pings the target every 2s with his server-side
	// position and distance, so we can tell whether Tick/movement is
	// actually running even if nothing visibly moves on your screen.
	// Tell me once confirmed and I'll strip this out.
	if current%40 == 0 {
		target.Message("DK debug: pos=", pos, " dist=", dist)
	}

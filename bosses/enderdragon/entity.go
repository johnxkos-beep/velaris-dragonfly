...
const (
	...
	diveCooldownBase   = 280  // ticks between dives (~14s) + jitter below — vanilla dives are fairly rare, not constant
	...
	breathCooldownBase   = 240 // ticks between breath attacks (~12s) + jitter below
...
// ground) on top of the centre tower.
//
...
func (e *EnderDragon) finishDeath(tx *world.Tx) {
	...
	eggBlockPos := cube.Pos{
		int(math.Floor(st.PerchPos.X())),
		int(st.PerchPos.Y()),
		int(math.Floor(st.PerchPos.Z())),
	}

...
//     all; the likely reason is that pack's render controllers gate on
//     query.death_ticks, a MoLang query tied to the vanilla server's own
//     dragon-death bookkeeping, which Dragonfly never populates — if that
//     query errors, no render controller matches and the whole model
//     disappears. The fix was rebuilding the client entity with a plain
//     "controller.render.default" (same safe pattern the crystal already
//     uses) and no special queries, at the cost of losing that pack's
//     fancy neck/tail/wing physics animation — she renders in a static
//     bind pose for now. See velaris_ender_dragon.mcpack.
//

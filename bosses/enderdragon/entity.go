...
//
// UNLIKE bosses/demonking, this uses the REAL vanilla Bedrock identifiers
// ("minecraft:ender_dragon", "minecraft:end_crystal") instead of a custom
// add-on model — the Ender Dragon and End Crystal both ship in every
// Bedrock client already, so no resource pack is needed for players to see
// the real model/animations. See the ENGINE-LEVEL CAVEAT below for what
// that trade-off costs us.
//
// UNVERIFIED — IDENTIFIER COLLISION RISK: this assumes Dragonfly's own
// dfentity.DefaultRegistry does NOT already define a built-in type for
// "minecraft:ender_dragon" or "minecraft:end_crystal" (it doesn't ship
// vanilla-parity mobs, so this should be safe — Demon King and every mob in
// this repo needed a from-scratch entity for the same reason). If
// EntityRegistry() below panics or `go build` complains about a duplicate
// identifier, paste the error — the fix is switching EncodeEntity to a
// custom identifier (e.g. "velaris:ender_dragon") and shipping a resource
// pack the way Demon King does, everything else here stays the same.
//
...
func newState(center mgl64.Vec3, radius, height float64, crystals *crystalCounter) *fightState {
	return &fightState{
		Phase: phaseCircling, HP: dragonMaxHP, MaxHP: dragonMaxHP,
		Center: center, Radius: radius, Height: height,
		Speed: orbitAngularSpeed, Crystals: crystals,
	}
}
...
func (dragonType) EncodeEntity() string { return "minecraft:ender_dragon" }
...
func (dragonType) Open(tx *world.Tx, handle *world.EntityHandle, data *world.EntityData) world.Entity {
	...
	if !ok || st == nil {
		st = newState(mgl64.Vec3{}, 35, 40, newCrystalCounter(0))
		data.Data = st
	}
...
func decodeDragonNBT(m map[string]any, data *world.EntityData) {
	...
	// NOTE: Center/Radius/Height/Crystals are deliberately NOT restored
	// from NBT — the crystal counter is an in-memory pointer shared with
	// live crystal entities and can't round-trip through save data. A
	// dragon reloaded mid-fight (server restart) will keep her HP but stop
	// regenerating and orbit around the world origin instead of the arena
	// centre. Acceptable for now; flag it if that's a problem in practice.
	data.Data = st
}
...
// radius/height and healed by crystals (pass the same *crystalCounter given
// to the arena's End Crystals).
type Config struct {
	Center           mgl64.Vec3
...
func (c Config) Apply(data *world.EntityData) {
	data.Data = newState(c.Center, c.Radius, c.Height, c.Crystals)
}

// Spawn creates and adds an Ender Dragon to tx, orbiting centre.
func Spawn(tx *world.Tx, centre mgl64.Vec3, radius, height float64, crystals *crystalCounter) *EnderDragon {
	pos := centre.Add(mgl64.Vec3{radius, height, 0})
	handle := world.EntitySpawnOpts{Position: pos}.New(Type, Config{Center: centre, Radius: radius, Height: height, Crystals: crystals})
...
// tickCircling advances the orbit and may kick off a dive or breath attack.
...
func (e *EnderDragon) finishDeath(tx *world.Tx) {
	st := e.fight
	st.Phase = phaseDead
	eggPos := st.Center.Add(mgl64.Vec3{0, 1, 0})

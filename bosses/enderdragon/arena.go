...
const (
	...
	podiumTowerHeight = 3  // the 1-wide bedrock tower the dragon perches on / the egg lands on
...
// buildTower places a 1-wide, podiumTowerHeight-tall bedrock tower rising
// straight from the natural ground at the arena centre — where the dragon
// periodically lands (see entity.go's perch phase) and where the Dragon Egg
// ends up on top of after death. No separate flat disc/platform around it
// (an earlier version had one — it read as an artificial-looking platform
// rather than a slender tower rising out of the terrain, which is what was
// actually wanted). Digs down to real ground first via findGround, same as
// buildPillar, rather than assuming the terrain is flat at the player's
// standing height. Not a functioning exit portal (deferred, per earlier
// request) — no end_portal blocks, just the tower itself.
//
// Returns the Y of the tower's top surface (where the egg/perch point is).
func buildTower(tx *world.Tx, base cube.Pos) int {
	groundY := findGround(tx, base.X(), base.Y(), base.Z())
	for y := groundY; y < groundY+podiumTowerHeight; y++ {
		tx.SetBlock(cube.Pos{base.X(), y, base.Z()}, block.Bedrock{}, nil)
	}
	return groundY + podiumTowerHeight
}

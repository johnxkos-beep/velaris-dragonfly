...
// disc + 3-block centre tower (the thing the dragon periodically lands on,
// and where her egg ends up after death — see finishDeath in entity.go),
// then spawns the Ender Dragon orbiting above it all. centre's Y is treated
// as the arena floor level — run this standing on the End island's surface.
//
...
func BuildArena(tx *world.Tx, centre mgl64.Vec3) *EnderDragon {
	...
	// where the dragon perches and the egg ends up — is at this Y.
	perchY := float64(base.Y() + 1 + podiumTowerHeight)
	perchPos := mgl64.Vec3{centre.X() + 0.5, perchY, centre.Z() + 0.5}

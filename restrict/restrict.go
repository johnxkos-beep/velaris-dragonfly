...
type Config struct {
	...
	wasInside       map[string]bool      // XUID -> already pushed for this zone visit (see enforcer.Tick)
}
...
func Load(path string) (*Config, error) {
	c := &Config{path: path, pending: map[string]*claim{}, lastWarn: map[string]time.Time{}, wasInside: map[string]bool{}}
...
func (e *enforcer) Tick(tx *world.Tx, _ int64) {
	...
	for p := range state.Server.Players(tx) {
		...
		stillInside[p.XUID()] = true

		// Only push on the tick they first cross into the zone, not
		// every tick they remain inside. Reapplying a large SetVelocity
		// 20 times/second stacks into a huge, repeated, unnatural
		// velocity injection — exactly what Bedrock clients treat as
		// speed-hack movement and self-disconnect over (the client-side
		// "Block"/ClientDisconnection-90 error this shipped with).
		// Vanilla knockback in this same codebase (knockback/config.go)
		// only ever applies a single 0.4 impulse per hit; matching that
		// shape — one impulse per zone entry, not a sustained force —
		// is what keeps this from looking like an exploit to the
		// client.
		if !Cfg.wasInside[p.XUID()] {
			Cfg.wasInside[p.XUID()] = true
	...
	// Clear the "already pushed" flag for anyone no longer inside any
	// zone, so their next entry gets a fresh impulse.
	for xuid := range Cfg.wasInside {
		if !stillInside[xuid] {
			delete(Cfg.wasInside, xuid)
		}
	}
	Cfg.mu.Unlock()
}

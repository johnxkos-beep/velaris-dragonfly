...
func (c *Config) HandleMove(ctx *player.Context, p *player.Player, newPos mgl64.Vec3) {
	...
	const strength = 0.6
	vel := mgl64.Vec3{(dx / length) * strength, 0.25, (dz / length) * strength}

	// Cancelling the move above already finishes the transaction it was
	// running in. SetVelocity and Message both need a *live* transaction
	// internally (e.g. to notify nearby viewers) — calling them
	// synchronously right here, on the now-finished transaction, is
	// exactly what paniced with "use of transaction after transaction
	// finishes is not permitted" the first time this shipped.
	//
	// Fix: do that work in a fresh transaction instead, via
	// EntityHandle.ExecWorld. That call has to happen from its own
	// goroutine rather than inline here — calling ExecWorld on a handle
	// while still inside another Exec on that same handle can deadlock
	// (see https://github.com/df-mc/dragonfly/issues/966 for the same
	// shape of bug), and HandleMove is itself running inside one.
	handle := p.H()
	go handle.ExecWorld(func(tx *world.Tx, e world.Entity) {
		pl, ok := e.(*player.Player)
		if !ok {
			return
		}
		pl.SetVelocity(vel)
		pl.Message("§cThis area is restricted to ops.")
	})
}

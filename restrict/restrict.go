...
// wallMinY/wallMaxY are the vertical extent of the wall — from just
// above bedrock up to the build height limit, independent of whatever Y
// the two corners were actually placed at. This world's actual bedrock
// sits at Y=-63 (confirmed in-game, not vanilla's usual -64), so
// wallMinY is -62 — one above it — so the wall never overwrites bedrock
// itself. If the top ever needs adjusting too, wallMaxY is the one to
// change; nothing else needs to.
const (
	wallMinY = -62
	wallMaxY = 319
)

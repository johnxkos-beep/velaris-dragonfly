...
func (c *Config) HandleMove(ctx *player.Context, p *player.Player, newPos mgl64.Vec3) {
	...
	// SetVelocity/Message have to run before ctx.Cancel(), not after.
	// Both need a *live* transaction internally (e.g. to notify nearby
	// viewers), and calling ctx.Cancel() finishes the transaction this
	// handler is running in — call them after Cancel() and they panic
	// with "use of transaction after transaction finishes is not
	// permitted", which is exactly what happened the first time this
	// shipped (deferring the work to a fresh transaction via
	// EntityHandle.ExecWorld was the first fix attempted, but that
	// method doesn't exist in the Dragonfly version this repo is pinned
	// to — v0.11.1 only has an unexported execWorld). Simplest fix:
	// don't fight the transaction at all, just do the tx-dependent work
	// while it's still valid, then cancel last.
	p.SetVelocity(vel)

...
func (a Activate) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	// Deliberately NOT spawning/confirming this zone's ticker here.
	// An earlier version called SpawnZoneTicker from right in this
	// handler, using this command's own tx — that pairing was never
	// independently confirmed safe, and since the same call crashed the
	// server outright when made from HandleBlockPlace's tx (see
	// koth/ticker.go's package doc comment), it was removed here too
	// rather than risk the same failure from a second untested call
	// site. Ticker spawning happens only from the player-join handler in
	// main.go now, via koth.EnsureAllZoneTickers — a zone gets its
	// ticker the next time anyone joins, not the moment it's activated.

...
func (a Activate) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	// this fix shipped its first-ever ticker.
	//
	// Spawn it at the EXECUTING PLAYER'S OWN position, never at
	// zone.SpawnPosition() (the zone's computed center) — the center can
	// land in a chunk this tx doesn't already have loaded/locked, and
	// Dragonfly's deferred handling of that case is exactly what crashed
	// the server ("use of transaction after transaction finishes"). The
	// op's own position is always safe, being inside whatever this tx
	// already has locked. In practice this means: for a zone that
	// already existed before this fix and so never got a ticker at
	// corner-completion time, stand at (or reasonably near) it before
	// running /koth activate the first time — if the op runs this from
	// somewhere unrelated to the zone entirely, the ticker still spawns
	// safely, just anchored wherever they happen to be standing instead
	// of at the zone, which only matters if they then leave and nobody
	// else comes near either spot (see koth.go's package doc comment).
	// Console-run activations skip this (no position to spawn at) and
	// rely on some future in-game activation to establish the ticker.
	if p, isPlayer := src.(*player.Player); isPlayer {
		if zone, zok := Cfg.ZoneByName(a.ID); zok {
			SpawnZoneTicker(tx, p.Position(), zone.Corner1)
		}
	}

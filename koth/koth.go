...
//
// BUGFIX (per-zone tickers, not one global one): the first version of
// this file spawned a single ticker entity once, wherever the first
// player of the session happened to be standing, and had it scan every
// active zone by name each tick. That meant capturing silently never
// worked: Dragonfly only calls Tick() on an entity while the chunk it's
// in is loaded, and loaded chunks track online players — not an
// arbitrary KOTH zone somewhere else on the map. Once whoever triggered
// that first spawn wandered away from that spot, the ticker's own chunk
// unloaded and it simply never ticked again for the rest of the session,
// with no error. Now there's one ticker per zone, spawned AT that zone
// (see ticker.go's SpawnZoneTicker, called from both a zone's corner
// completion and from /koth activate) — so the same thing that keeps a
// zone's chunk loaded for a player to stand in it also keeps that zone's
// own ticker loaded and running, exactly when it's needed. Each ticker
// is tracked by its zone's first corner position (Config.FindByCorner),
// a stable key that survives naming and repeated activation.
package koth
...
// SpawnPosition returns a point at the zone's own footprint center, just
// above its highest corner.
//
// NOT currently used to spawn this zone's ticker (see ticker.go's
// SpawnZoneTicker) — an earlier revision did exactly that and it crashed
// the server: this point can land in a chunk that isn't already
// loaded/locked under whatever *world.Tx happens to be spawning it, and
// Dragonfly's handling of that case ("use of transaction after
// transaction finishes is not permitted") is a hard panic, not a soft
// failure. Both real call sites (players.go's HandleBlockPlace and
// command.go's Activate.Run) spawn at the acting player's own current
// position instead, which is always guaranteed safe. Kept as a small
// utility in case something else (a teleport command, a map marker)
// wants "the middle of this zone" without needing tx-safety guarantees.
func (z Zone) SpawnPosition() mgl64.Vec3 {
	cx, cz, topY, _, _ := z.center()
	return mgl64.Vec3{cx, float64(topY + 1), cz}
}
...
type Config struct {
	...
	// spawnedTickers records which zones (keyed by first corner — see
	// FindByCorner) already have a per-zone ticker entity spawned this
	// session, so corner-completion and /koth activate (which both call
	// SpawnZoneTicker) don't race and spawn two for the same zone. Not
	// persisted — a fresh boot re-derives this from scratch as each
	// zone's own already-in-the-world ticker entity reloads with its
	// chunk (see ticker.go's EncodeNBT/DecodeNBT) or, for zones whose
	// ticker predates this fix, the first /koth activate after upgrading
	// spawns a fresh one.
	spawnedTickers map[cube.Pos]bool
}
...
//
// BUGFIX HISTORY: an earlier revision also returned the just-completed
// Zone here, and the caller (players.go) used it to spawn that zone's
// ticker immediately, from right inside HandleBlockPlace. That crashed
// the server: HandleBlockPlace runs several frames deep inside
// Dragonfly's block-placement/inventory-transaction handling, which
// operates through a "weak" transaction reference (visible in a crash
// stack as EntityHandle.runScheduled/execWorld/weakTransaction.Run) —
// and world.Tx.AddEntity cannot be called from there at all, regardless
// of what position is passed to it; it panics with "use of transaction
// after transaction finishes is not permitted" every time, immediately.
// Ticker spawning now happens only from EnsureAllZoneTickers (see
// ticker.go), called from the ordinary player-join handler in main.go —
// the same context border.EnsureTicker/countdown.EnsureTicker/etc.
// already use successfully in this codebase. See ticker.go's package
...
func (c *Config) OnBlockPlace(xuid string, pos cube.Pos, b world.Block) (msg string, ok bool) {
	...
	return "§aKOTH zone shape complete! Name it with §e/koth name <id> — its ticker will start the next time someone joins the server.", true
}
...
// no winner). It also checks completed-but-not-yet-named zones, so
// breaking a corner before naming it cleans up rather than leaving a
// half-defined zone (and its already-spawned ticker — see
// SpawnZoneTicker — to notice and quietly despawn itself) hanging
// around forever. ok is false if pos wasn't a zone corner at all.
func (c *Config) OnBlockBreak(pos cube.Pos) (msg string, ok bool) {
	...
	for i, z := range c.pendingUnnamed {
		if z.Corner1 == pos || z.Corner2 == pos {
			c.pendingUnnamed = append(c.pendingUnnamed[:i], c.pendingUnnamed[i+1:]...)
			return "§eUnfinished KOTH zone shape removed.", true
		}
	}
	return "", false
}

// FindByCorner looks up a zone — named or still awaiting /koth name — by
// its first corner, the stable identity a per-zone ticker tracks itself
// by across naming and repeated activation (see ticker.go's Tick).
// exists is false if pos isn't the first corner of anything right now
// (the zone was never completed with this corner, or it's since been
// broken — see OnBlockBreak). named is false for a completed-but-not-yet
// -named zone, in which case active is always nil.
func (c *Config) FindByCorner(corner1 cube.Pos) (name string, zone Zone, active *activeState, named, exists bool) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	for n, z := range c.d.Zones {
		if z.Corner1 == corner1 {
			return n, z, c.active[n], true, true
		}
	}
	for _, z := range c.pendingUnnamed {
		if z.Corner1 == corner1 {
			return "", z, nil, false, true
		}
	}
	return "", Zone{}, nil, false, false
}

// AllCorners returns the first corner of every zone this Config knows
// about right now — named or still awaiting /koth name. Used by
// EnsureAllZoneTickers (ticker.go) to find every zone that might still
// need its ticker spawned.
func (c *Config) AllCorners() []cube.Pos {
	c.mu.RLock()
	defer c.mu.RUnlock()
	out := make([]cube.Pos, 0, len(c.d.Zones)+len(c.pendingUnnamed))
	for _, z := range c.d.Zones {
		out = append(out, z.Corner1)
	}
	for _, z := range c.pendingUnnamed {
		out = append(out, z.Corner1)
	}
...
// MarkTickerSpawned records that a ticker entity now exists for the zone
// whose first corner is corner1, returning true the first time it's
// called for a given corner (the caller should go ahead and spawn one)
// and false every time after (one already exists this session — see
// SpawnZoneTicker, which is the only caller and only spawns when this
// returns true). This only guards against spawning two at once in the
// same session, e.g. if corner-completion and /koth activate both race
// to spawn a brand new zone's ticker — it isn't persisted, since each
// zone's ticker is a real world entity that reloads itself with its own
// chunk on a fresh boot (see ticker.go's EncodeNBT/DecodeNBT).
func (c *Config) MarkTickerSpawned(corner1 cube.Pos) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.spawnedTickers[corner1] {
		return false
	}
	c.spawnedTickers[corner1] = true
	return true
}

// ZoneByName returns the named zone's stored shape — for callers (like
// /koth activate) that need its corners to do something with the real
// world, such as spawning its ticker at its center via SpawnZoneTicker.
func (c *Config) ZoneByName(name string) (Zone, bool) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	z, ok := c.d.Zones[name]
	return z, ok

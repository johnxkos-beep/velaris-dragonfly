...
// part of an active selection. completed is non-nil exactly when this
// call was the second (shape-completing) corner — the caller should pass
// it straight to SpawnZoneTicker (with a real *world.Tx, which this
// package-internal method has no access to) so the zone's ticker exists
// from the moment it physically exists, instead of waiting for /koth
// activate. See players.go's HandleBlockPlace in INTEGRATION.md.
func (c *Config) OnBlockPlace(xuid string, pos cube.Pos, b world.Block) (msg string, ok bool, completed *Zone) {
	if !isMarkerBlock(b) {
		return "", false, nil
	}
	...
	if !cl.hasFirst {
		cl.hasFirst = true
		cl.first = pos
		return "§aFirst KOTH zone corner set. Place the second block to complete the zone.", true, nil
	}

	zone := Zone{Corner1: cl.first, Corner2: pos, Owner: xuid}
	c.pendingUnnamed = append(c.pendingUnnamed, zone)
	delete(c.pending, xuid)
	return "§aKOTH zone shape complete! Name it with §e/koth name <id>", true, &zone
}

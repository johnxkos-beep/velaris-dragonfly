...
func SpawnZoneTicker(tx *world.Tx, pos mgl64.Vec3, corner1 cube.Pos) {
	...
	tx.AddEntity(handle)
	// Mirrors border's own "[border] ticker entity spawned at ..." log
	// line — the one visible way to confirm from the console that a
	// zone's ticker actually exists this session, since (unlike
	// block-place protection, which is checked live on every event)
	// nothing else prints when a zone has no ticker at all; it just
	// silently never captures. If you activate a zone and never see a
	// matching "[koth] ticker spawned" line for it anywhere in the log,
	// its ticker doesn't exist yet — see EnsureAllZoneTickers' doc
	// comment for when it gets created.
	log.Printf("[koth] ticker spawned for zone at corner %v, anchored at %v", corner1, pos)
}

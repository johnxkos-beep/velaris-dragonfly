...
//
// pos MUST be a position already guaranteed safe under tx — in practice,
// the calling player's own current position, never a separately computed
// point like Zone.SpawnPosition(). Dragonfly has to reach into whatever
// chunk pos falls in, and if that chunk isn't already loaded/locked
// under tx, it defers the work — and if that deferred work runs after
// tx has already closed (which it usually has, by the time it's most
// likely to matter), it's a hard panic ("use of transaction after
// transaction finishes is not permitted"), not a soft failure. This
// crashed the server the first time this shipped, using the zone's own
// center — see Zone.SpawnPosition's doc comment for the postmortem.
//
// There are two call sites, meant to fire for the same zone at different
// times without double-spawning: players.go's HandleBlockPlace calls
// this the instant a zone's second corner completes its shape (so the
// ticker exists immediately, before it even has a name), and
// command.go's Activate.Run also calls it every time /koth activate
// runs (so a zone that already existed before this fix shipped — and so
// never got a ticker at corner-completion time, because that already
// happened in the past under the old code — gets one the first time

...
// around obstacles. Step height is bounded to 1 block via resolveMobStep
// (shared with mob.go — see that file's doc comment for the full
// explanation of the wall-climbing bug it fixes): a mob mid-chase that
// hits something taller than one step just stands there facing the
// target instead of climbing over it or getting stuck mid-air.
//
...
const (
	...
	// hostileRetargetTicks throttles the EXPENSIVE part of findTarget: the
	// full online-player scan that runs whenever a mob currently has no
	// valid target. A mob that's already chasing someone skips that scan
	// entirely (single O(1) handle check instead) — this only matters for
	// idle/untargeted mobs, but there can be a lot of those right after
	// night falls with 100 players' worth of spawns going off at once. At
	// 5 ticks (~0.25s) a still-idle mob acquires a target within a
	// quarter-second of someone walking into range — imperceptible — while
	// cutting that scan's frequency 5x. See RetargetCooldown/newHostileState
	// for the per-mob stagger that spreads these scans across the window
	// instead of bunching them on the same tick.
	hostileRetargetTicks = 5
)
...
type hostileState struct {
	...
	WanderTicks    int
	IdleTicks      int
	RetargetCooldown int // ticks until the next full-player-scan retarget attempt; see hostileRetargetTicks
}

func newHostileState(spec *hostileSpec) *hostileState {
	return &hostileState{
		HP: spec.MaxHP, MaxHP: spec.MaxHP, Speed: spec.WalkSpeed,
		// Staggered so 100+ mobs spawned on the same tick don't all
		// re-scan for a target on the same tick too — see
		// RetargetCooldown's use in findTarget below.
		RetargetCooldown: rand.Intn(hostileRetargetTicks),
	}
}
...
func (e *HostileMob) Tick(tx *world.Tx, current int64) {
	...
	default: // attackMelee, attackRanged — same chase-then-hit-on-cooldown shape
		if dist <= e.spec.AttackRange {
			...
			target := pos.Add(step)
			if newPos, ok := resolveMobStep(tx, pos, target.X(), target.Z()); ok {
				e.data.Pos = newPos
			}
			// Blocked by something taller than one step: this mob has
			// no wander fallback mid-chase (unlike passive mobs), so it
			// just stands there facing the target until dist/line
			// changes enough to unstick it next tick — acceptable given
			// the existing "no pathfinding" caveat this file already
			// documents.
		}
	}
...
func (e *HostileMob) tickExplode(tx *world.Tx, target *player.Player, dist float64) {
	...
	if step.Len() > 0.01 {
		...
		next := e.data.Pos.Add(step)
		if newPos, ok := resolveMobStep(tx, e.data.Pos, next.X(), next.Z()); ok {
			e.data.Pos = newPos
		}
	}
}
...
// identical shape to demonking.findTarget, PLUS a throttle demonking
// doesn't need: demonking is a single boss instance, but there can be
// well over a hundred of these mobs at once, so the "I have no target,
// scan every online player" branch only runs once every
// hostileRetargetTicks per mob (staggered — see RetargetCooldown) instead
// of every tick. The "I already have a target" fast path below is
// unaffected — it was already O(1), not the expensive part.
func (e *HostileMob) findTarget(tx *world.Tx) *player.Player {
	...
	if e.state.RetargetCooldown > 0 {
		e.state.RetargetCooldown--
		return nil
	}
	e.state.RetargetCooldown = hostileRetargetTicks
...
func (e *HostileMob) wander(tx *world.Tx) {
	...
	if st.WanderTicks > 0 {
		...
		if newPos, ok := resolveMobStep(tx, e.data.Pos, target.X(), target.Z()); ok {
			...
			st.WanderTicks = 0 // blocked — pick a fresh direction next cycle
		}
		e.broadcastMove(tx)
		return
	}

...
// area/global population caps (areaMobCap/globalMobCap), same
// ground-validity check (validSpawnGround/findGroundNear), and the same
// "re-home an invisible ticker entity to a live player every tick so its
// own chunk never unloads" trick. All of those are reused directly from
// spawning.go (same package) rather than copy-pasted, so the two

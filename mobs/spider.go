...
//
// The string item is looked up at runtime via world.ItemByName("minecraft:string", 0)
// rather than a guessed Go struct (item.String{}) — build confirmed
// item.String{} doesn't exist in this pinned Dragonfly version (same
// naming drift already hit for wool in sheep.go; see that file's comment
// for the fuller rationale). world.ItemByName is the item-package
// equivalent of world.BlockByName, keyed off the wire-protocol ID string
// instead of a Go type name that can drift between versions.
// Spider eye deliberately left out: it's a two-word vanilla item and I'd
// be guessing blind at its Bedrock ID string too — string is the vast
// majority of a spider's drop table anyway. Easy to add once confirmed.
func spiderDrops() []item.Stack {
	n := rand.Intn(3) // 0, 1, or 2
	if n == 0 {
		return nil
	}
	it, ok := world.ItemByName("minecraft:string", 0)
	if !ok {
		return nil
	}
	return []item.Stack{item.NewStack(it, n)}
}

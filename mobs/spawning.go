...
//
// DEBUGGING: every successful natural spawn prints a line to the console
// (visible in the Pterodactyl console tab) — see the fmt.Printf call in
// attemptSpawn below. If you watch the console for a couple minutes and
// see nothing at all, the spawner isn't running (chunk unloaded, or
// something failed silently) — that's a different problem from "it's
// running but I can't find the mobs". If you DO see spawn lines but never
// find the animals, they're most likely just out of sight — spawnMaxRadius
// (see below) puts them up to spawnMaxRadius blocks away, easily behind
// terrain or outside your render distance, and they wander slowly
// (~0.045-0.05 blocks/tick, under 1 block/sec) so they won't quickly
...
const (
	...
	spawnMinRadius         = 8.0  // blocks from the player — close enough to actually spot
	spawnMaxRadius         = 16.0 // blocks from the player — was 28, pulled in for visibility
...
func (s *spawner) Tick(tx *world.Tx, current int64) {
	...
	if n := countMobs(tx); n >= globalMobCap {
		fmt.Printf("[mobs] spawn cycle skipped: global cap reached (%d mobs)\n", n)
		return
	}
...
func attemptSpawn(tx *world.Tx, p *player.Player) {
	...
	if !ok {
		fmt.Printf("[mobs] spawn attempt near %s failed: no valid ground found\n", p.Name())
		return
	}
	if n := countMobsWithin(tx, pos, localClusterRadius); n >= localClusterCap {
		fmt.Printf("[mobs] spawn attempt near %s skipped: local cap reached (%d nearby)\n", p.Name(), n)
		return
	}

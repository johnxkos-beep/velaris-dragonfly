...
func EntityTypes() []world.EntityType {
	return []world.EntityType{
		CowType, ChickenType, PigType, SheepType, SpawnerType,
		ZombieType, SkeletonType, SpiderType, CreeperType, HostileSpawnerType,
	}
}
...
func Register() {
	...
	zombieStack := item.NewStack(ZombieSpawnEgg{}, 1)
	skeletonStack := item.NewStack(SkeletonSpawnEgg{}, 1)
	spiderStack := item.NewStack(SpiderSpawnEgg{}, 1)
	creeperStack := item.NewStack(CreeperSpawnEgg{}, 1)

	// One shared group for all eggs, shown in the Items tab (matches
	...
	creative.RegisterItem(creative.Item{Stack: zombieStack, Group: creativeGroupName})
	creative.RegisterItem(creative.Item{Stack: skeletonStack, Group: creativeGroupName})
	creative.RegisterItem(creative.Item{Stack: spiderStack, Group: creativeGroupName})
	creative.RegisterItem(creative.Item{Stack: creeperStack, Group: creativeGroupName})
}

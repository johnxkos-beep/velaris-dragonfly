...
// path around obstacles, and will simply stop (and pick a new direction
// next cycle) if the way ahead is blocked by anything taller than one
// block — see resolveMobStep below.
//
// STEP HEIGHT / WALL CLIMBING: movement used to call snapToGround, which
// searched up to 3 blocks ABOVE the mob's current feet for the nearest
// solid surface and snapped straight onto it — meaning walking into the
// base of any wall up to 3 blocks tall instantly teleported the mob on
// top of it, since the search never checked whether that height was
// actually reachable by stepping, only whether solid ground existed
// somewhere within reach. resolveMobStep (shared with hostilemob.go)
// replaces that: it only allows stepping up exactly 1 block (matching
// vanilla mobs' normal auto-step height) and, when nothing at same-level,
// one-block-up, or lower is walkable, reports the move as blocked instead
// of snapping upward — the caller then just leaves the mob where it is.
//
// PERFORMANCE: everything in Tick is O(1) — a handful of int/float
// comparisons and, when moving, a couple of block lookups (resolveMobStep)
...
func (e *Mob) Tick(tx *world.Tx, current int64) {
	...
	if e.data.Vel.Len() > 0.03 {
		target := e.data.Pos.Add(e.data.Vel)
		if newPos, ok := resolveMobStep(tx, e.data.Pos, target.X(), target.Z()); ok {
			e.data.Pos = newPos
		}
	...
	if st.FleeTicks > 0 {
		...
		target := e.data.Pos.Add(step)
		if newPos, ok := resolveMobStep(tx, e.data.Pos, target.X(), target.Z()); ok {
			e.data.Pos = newPos
		} else {
			// Blocked by something taller than one step — stop this
			// flee burst instead of repeatedly bumping into the wall;
			// the next Hurt() call (or a fresh wander decision once
			// FleeTicks runs out) will pick a new direction.
			st.FleeTicks = 0
		}
	...
	if st.WanderTicks > 0 {
		...
		if newPos, ok := resolveMobStep(tx, e.data.Pos, target.X(), target.Z()); ok {
			...
			// Blocked — end this wander burst early so pickNextAction
			// chooses a fresh direction next cycle instead of walking
...
// resolveMobStep computes the landing position for a mob moving from
// current toward (targetX, targetZ), allowing at most a 1-block step UP
// (matching typical mob auto-step height) — the fix for mobs being able
// to "climb" walls/ledges taller than that (see the file doc comment).
// Stepping/falling DOWN is still unbounded — a mob can walk off a ledge
// and land several blocks down in one tick, same simplified no-fall-
// animation behaviour as before; that was never the reported bug.
// Shared by mob.go (passive) and hostilemob.go (hostile) — both call
// this instead of keeping their own copy.
//
// Returns ok=false if nothing valid is found at all (blocked ahead by
// something taller than one step, with no reasonable drop either); the
// caller should then leave the mob at its current position rather than
// move it.
func resolveMobStep(tx *world.Tx, current mgl64.Vec3, targetX, targetZ float64) (mgl64.Vec3, bool) {
	feetY := int(math.Floor(current.Y()))
	bx := int(math.Floor(targetX))
	bz := int(math.Floor(targetZ))

	// Same level: clear headroom at the current feet height, with solid
	// ground directly beneath it — just walk forward, no step needed.
	if isAirBlock(tx, bx, feetY, bz) && isAirBlock(tx, bx, feetY+1, bz) && !isAirBlock(tx, bx, feetY-1, bz) {
		return mgl64.Vec3{targetX, float64(feetY), targetZ}, true
	}

	// Step up exactly 1 block — the ONLY upward case allowed, no matter
	// how tall whatever's ahead actually is.
	if !isAirBlock(tx, bx, feetY, bz) && isAirBlock(tx, bx, feetY+1, bz) && isAirBlock(tx, bx, feetY+2, bz) {
		return mgl64.Vec3{targetX, float64(feetY + 1), targetZ}, true
	}

	// Step/fall down onto the first solid ground below — unbounded,
	// same as the old snapToGround's downward behaviour.
	for i := 1; i <= mobStepFallSearchRange; i++ {
		y := feetY - i
		if !isAirBlock(tx, bx, y, bz) && isAirBlock(tx, bx, y+1, bz) {
			return mgl64.Vec3{targetX, float64(y + 1), targetZ}, true
		}
	}
...
// isAirBlock is a tiny shorthand used throughout resolveMobStep.
func isAirBlock(tx *world.Tx, x, y, z int) bool {
	_, ok := tx.Block(cube.Pos{x, y, z}).(block.Air)
	return ok
}

// mobStepFallSearchRange bounds how far down resolveMobStep searches for
// a landing spot when stepping/falling off a ledge.
const mobStepFallSearchRange = 20

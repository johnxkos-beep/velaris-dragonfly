...
func CleanEntities(worldDir string, dim world.Dimension, centerX, centerZ, radiusChunks int, reg world.EntityRegistry, log *slog.Logger) (removed int, err error) {
	// The world's leveldb LOCK is briefly held by the previous container
	// instance for a moment even after Docker reports it "Exited" — retry
	// a few times with backoff instead of failing on the first attempt,
	// since this consistently self-resolves within a couple of seconds.
	var db *mcdb.DB
	for attempt := 1; attempt <= 24; attempt++ {
		...
		log.Info("world save still locked, retrying", "attempt", attempt, "error", err.Error())
		time.Sleep(5 * time.Second)
	}
	if err != nil {
		return 0, fmt.Errorf("open world %q after retries: %w", worldDir, err)
	}
	defer func() {
		if cerr := db.Close(); cerr != nil {
			log.Error("world entity cleanup: failed to close world cleanly — this leaves the world locked for the rest of this process, which is why the real server fails to start right after", "error", cerr.Error())
		}
	}()

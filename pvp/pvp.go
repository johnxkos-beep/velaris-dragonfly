...
// /pvp off, and /pvp block commands — all three are op-only.
//
// /pvp on and /pvp off flip PvP for the whole server: on means anyone can
// fight anyone anywhere, off means nobody can fight outside a PvP zone.
// /pvp block gives the executor exactly 2 marker blocks. Placing the first
// marks corner 1; placing the second marks corner 2 and turns the cuboid
// between them into a PvP zone, where combat is always allowed regardless
// of the server-wide on/off state. Breaking either corner block deletes
// the zone.
package pvp
...
type data struct {
	Enabled bool   `json:"enabled"` // server-wide PvP on/off
...
// Enabled reports whether PvP is currently on server-wide. Defaults to
// false until an op runs /pvp on.
...
// SetEnabled sets the server-wide PvP state and persists it. Called by
// /pvp on and /pvp off (op-only).
func (c *Config) SetEnabled(on bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.d.Enabled = on
	_ = c.save()
...
// CombatAllowed decides whether combat may happen at the given attacker
// position (at). PvP zones always allow combat regardless of the
// server-wide state; outside a zone, combat is only allowed while PvP is
// on server-wide (see Enabled/SetEnabled).
func (c *Config) CombatAllowed(at cube.Pos) bool {
	c.mu.RLock()
	defer c.mu.RUnlock()

...
// MarkerBlock is the block used for PvP zone corners — vanilla orange
// concrete, matching what the original PMMP plugin used. If Dragonfly
// turns out not to have a usable concrete/colour block, swap this one
// function out for whatever solid block you'd rather use (e.g.
// block.Wool{Colour: colour.Orange()}) — nothing else in this package
// needs to change.
//
// UNVERIFIED: block.Concrete{Colour: colour.Orange()} and the
// "github.com/df-mc/dragonfly/server/block/colour" package path are my
// best-confidence read of Dragonfly's block API — there's no local Go
// toolchain in this environment to confirm the exact type/field names
// against v0.11.1. If `go build` disagrees, the error will point straight
// at this line and it's a one-line fix.
func MarkerBlock() block.Concrete { return block.Concrete{Colour: colour.Orange()} }

// isMarkerBlock reports whether b is the PvP marker block.
func isMarkerBlock(b world.Block) bool {
	c, ok := b.(block.Concrete)
	return ok && c.Colour == colour.Orange()
}

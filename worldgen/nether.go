...
func (n Nether) GenerateChunk(pos world.ChunkPos, c *chunk.Chunk) {
	...
	wastes, wastesOK := world.BiomeByName("minecraft:nether_wastes")
	soulValley, soulValleyOK := world.BiomeByName("minecraft:soul_sand_valley")
	if !wastesOK {
		panic("worldgen: minecraft:nether_wastes biome not registered")
	}
	if !soulValleyOK {
		soulValley = wastes
	}

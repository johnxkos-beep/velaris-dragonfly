	...
	for lx := uint8(0); lx < 16; lx++ {
		for lz := uint8(0); lz < 16; lz++ {
			...
			for y := min; y < max; y++ {
				...
				// Bedrock floor caps the bottom of the dimension so nothing
				// (players included) falls forever if they clip below the
				// island — everything else below "top" and above this is
				// left as void/air, matching the real End's floating-island
				// look.
				if y == min {

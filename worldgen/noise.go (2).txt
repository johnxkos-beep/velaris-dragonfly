...
// mix runs a splitmix64-style finalizer over the input. This gives much
// better bit avalanche than a plain multiply-xor-shift, which matters here:
// the previous finalizer left visible diagonal banding/terracing in the
// generated terrain because nearby lattice points hashed to correlated
// values. Taking the top bits after this finalizer (instead of `% 1000000`,
// which throws away high-order entropy and reintroduces periodicity) fixes
// that.
...
func hash3(x, y, z int64, seed int64) float64 {
	h := uint64(x)*0x9E3779B97F4A7C15 + uint64(y)*0xC2B2AE3D27D4EB4F + uint64(z)*0xFF51AFD7ED558CCD + uint64(seed)*0x165667B19E3779F9
	h = mix(h)
	return float64(h>>11) / float64(1<<53)
}

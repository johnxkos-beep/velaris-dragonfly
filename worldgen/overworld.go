...
func (o Overworld) GenerateChunk(pos world.ChunkPos, c *chunk.Chunk) {
	// A panic anywhere in here runs on Dragonfly's shared world goroutine
	// with nothing above it to recover - it takes the whole server down
	// and kicks every connected player, not just whoever walked into the
	// unexplored chunk that triggered it. Recovering and logging instead
	// leaves that one chunk generated up to whatever point it reached
	// (worst case: partially air) rather than ending everyone's session.
	defer func() {
		if r := recover(); r != nil {
			logGenPanic("overworld", pos, r)
		}
	}()
	...
	// block.Snow{Layers: 1} didn't compile either (unknown field). Rather
	// than keep guessing field names blind, using the zero-value struct —
	// this compiles regardless of what fields block.Snow actually has.
	// Cosmetic-only (snow layer thickness in worldgen), not worth more
	// round-trips right now. If you want to tune the layer thickness
	// later, run `go doc github.com/df-mc/dragonfly/server/block Snow`
	// on your machine to see the real field name and I'll wire it in.
	snowLayerRID := world.BlockRuntimeID(block.Snow{})

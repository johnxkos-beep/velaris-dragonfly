...
func (o Overworld) GenerateChunk(pos world.ChunkPos, c *chunk.Chunk) {
	...
	// Note: "extreme_hills" was renamed to "windswept_hills" upstream; if a
	// biome name here ever stops matching again (another rename, etc.), we
	// fall back to plains below rather than crashing on a nil Biome.
	plains, plainsOK := world.BiomeByName("minecraft:plains")
	forest, forestOK := world.BiomeByName("minecraft:forest")
	desert, desertOK := world.BiomeByName("minecraft:desert")
	if !forestOK {
		forest = plains
	}
	if !desertOK {
	...
	hills, hillsOK := world.BiomeByName("minecraft:windswept_hills")
	ocean, oceanOK := world.BiomeByName("minecraft:ocean")
	if !plainsOK {
		panic("worldgen: minecraft:plains biome not registered")
	}
	if !hillsOK {
		hills = plains
	}

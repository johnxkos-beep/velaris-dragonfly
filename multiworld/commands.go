...
func (m MWTeleport) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	//
	// TEMPORARY DIAGNOSTIC: for a brand-new world, w.Spawn() may resolve to
	// a position inside solid/ungenerated terrain (e.g. underground),
	// which can cause an instant client-side disconnect on arrival with
	// nothing logged server-side. Hardcoding a safe high altitude (Y=100)
	// here to rule that out. If this fixes the kick, the real fix is
	// making sure new worlds get a proper generated-and-safe spawn point
	// before anyone can /mw tp to them — tell me and I'll wire that up
	// instead of this temporary override.
	spawn := w.Spawn()
	pos := mgl64.Vec3{float64(spawn.X()), 100, float64(spawn.Z())}

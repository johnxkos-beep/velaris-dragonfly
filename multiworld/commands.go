...
func (m MWTeleport) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	// ASSUMPTION FLAG: w.Spawn().Vec3() is my best read of how to get a
	// *world.World's spawn point without already being inside one of its
	// transactions (SetWorldSpawn in commands.go only shows the reverse —
	// setting it via tx.World().SetSpawn). If .Spawn() or .Vec3() isn't
	// right, paste me the error.
	pos := w.Spawn().Vec3()

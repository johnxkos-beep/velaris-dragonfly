...
func (m MWTeleport) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	// the recent Exec/Do churn) that converts it to mgl64.Vec3.
	//
	// Reverted the forced Y=100 — that was actually the cause of the
	// "player died" you just saw: this world's valid Y range apparently
	// doesn't reach 100, so the player took instant void/out-of-range
	// damage on arrival. The real disconnect bug (the world.Call deadlock
	// in TeleportTo) is fixed now, so the plain, real spawn point should
	// be safe again.
	pos := w.Spawn().Vec3()

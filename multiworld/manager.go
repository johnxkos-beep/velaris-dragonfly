...
func TeleportTo(tx *world.Tx, p *player.Player, w *world.World, pos mgl64.Vec3) {
	...
	// FIXED (previous attempt was wrong): world.Call's own docs say
	// explicitly "Calling it from the owner itself (any scheduled callback
	// or Handler event) deadlocks." This function runs INSIDE a command's
	// Run(), which is itself a scheduled callback on the source world's
	// owner goroutine (that's what the tx parameter is) — so calling
	// world.Call directly here, as the previous version did, was exactly
	// the deadlock case the docs warn about, and is almost certainly what
	// caused the instant, silent disconnect. Handing the destination-world
	// work off to its own goroutine makes it genuinely off-owner code,
	// which is what world.Call is meant to be used from.
	go func() {

...
func TeleportTo(tx *world.Tx, p *player.Player, w *world.World, pos mgl64.Vec3) {
	...
	// FIXED: World.Do() only schedules the re-add and returns immediately —
	// it does not wait for it to run. That left a real window where the
	// player existed in no world at all (removed here, not yet added on
	// w's own goroutine), which is almost certainly what caused the instant
	// disconnect you saw: Bedrock clients hard-disconnect when their own
	// entity disappears without an immediate replacement. world.Call blocks
	// until the callback actually finishes, closing that gap.

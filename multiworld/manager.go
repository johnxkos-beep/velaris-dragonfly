...
func TeleportTo(tx *world.Tx, p *player.Player, w *world.World, pos mgl64.Vec3) {
	...
	// cross-world work happens in its own goroutine here. This matches the
	// structure of RedStoneCraftGG/df-multiworld, a working reference
	// implementation of the same exact operation.
	//
	// FIXED: switched tx.AddEntityAt(handle, pos) to the reference
	// implementation's exact pattern — tx.AddEntity(handle) followed by a
	// separate p.Teleport(pos) call. AddEntityAt is a newer convenience
	// method; on a build this bleeding-edge it's a real candidate for
	// having its own bug even though AddEntity+Teleport (the older,
	// better-exercised path) works fine — which is exactly what the
	// working reference project uses.
	go func() {
		_, err := world.Call(context.Background(), w, func(tx *world.Tx) (struct{}, error) {
			ent := tx.AddEntity(handle)
			if ent == nil {
				return struct{}{}, fmt.Errorf("tx.AddEntity returned nil")
			}
			if np, ok := ent.(*player.Player); ok {
				np.Teleport(pos)
			}
			return struct{}{}, nil
		})

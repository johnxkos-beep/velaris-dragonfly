...
// ---------------------------------------------------------------------
// ASSUMPTION FLAG — read this before debugging build errors in this file.
//
// This file is the one part of this whole port I have real uncertainty
// about, because I don't have a Go toolchain or network access in my
// sandbox to check it against your exact pinned commit
// (df-mc/dragonfly v0.11.2-0.20260805...), which is a dev build newer than
// anything I have reliable training knowledge of.
//
// Two API surfaces here are my best-effort reconstruction rather than
// something I've verified:
//
//  1. world.Config{Dim, Generator, Provider, Log}.New() for opening a
//     standalone *world.World outside the server's built-in three
//     (Overworld/Nether/End) — main.go only shows the built-in three being
//     configured via server.Config, not this path.
//  2. Moving a player from one *world.World to another using the entity
//     handle (p.H(), tx.RemoveEntity, destWorld.Exec + tx.AddEntity) — this
//     is the general pattern Dragonfly uses internally for portals, but I
//     can't confirm the exact method names on this build.
//
// If either fails to compile, paste me the exact error for that function —
// I'll fix just that piece rather than guessing again blind. Everything
// else in this package (the registry, the generators, folder handling) has
// no such uncertainty.
// ---------------------------------------------------------------------
...
//
// See the ASSUMPTION FLAG at the top of this file — this is the function
// most likely to need a small fix against your exact dragonfly build.

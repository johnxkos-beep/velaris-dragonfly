...
func (n Nether) GenerateChunk(pos world.ChunkPos, c *chunk.Chunk) {
	...
	netherrackID, _ := world.BlockRuntimeID(block.Netherrack{})
	bedrockID, _ := world.BlockRuntimeID(block.Bedrock{})
	lavaID, _ := world.BlockRuntimeID(block.Lava{Depth: 8, Still: true})
	soulSandID, _ := world.BlockRuntimeID(block.SoulSand{})
	glowstoneID, _ := world.BlockRuntimeID(block.Glowstone{})
	quartzOreID, _ := world.BlockRuntimeID(block.NetherQuartzOre{})
...
// NewFlat creates a Flat generator. layers lists the block runtime IDs placed
// from the surface down (index 0 = topmost); everything below that down to
// (but not including) the bottom-most bedrock layer is filled with the last
// entry in layers.
func NewFlat(biome world.Biome, layers []world.Block) Flat {
	...
	f.bedrock, _ = world.BlockRuntimeID(block.Bedrock{})
	return f
}

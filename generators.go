...
func (SkyBlock) GenerateChunk(pos world.ChunkPos, c *chunk.Chunk) {
	...
	// ASSUMPTION FLAG: Log{Wood:...} / Leaves{Wood:...} and block.OakWood()
	// are my best read of Dragonfly's block package shape (same pattern as
	// this repo's worldgen using block.Stone{}/block.Air{}), but I can't
	// compile-check them here. If this line specifically fails to build,
	// paste me the error and I'll fix just this.
	logRID := world.BlockRuntimeID(block.Log{Wood: block.OakWood()})
	leavesRID := world.BlockRuntimeID(block.Leaves{Wood: block.OakWood()})

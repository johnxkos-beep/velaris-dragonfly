...
func (l Load) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	if LegacyExists(name) {
		clip, unknown, err := ReadMcEditSchematic(name)
		if err != nil {
			p.Messagef("§c%s", err)
			return
		}
		SetClipboard(p.XUID(), clip)
		if unknown > 0 {
			p.Messagef("§aLoaded §e%s§a (%dx%dx%d, %d blocks, §c%d unmapped -> air§a) into your clipboard. Run §e/paste§a to place it.", name, clip.Size.X, clip.Size.Y, clip.Size.Z, clip.Size.Volume(), unknown)
		} else {
			p.Messagef("§aLoaded §e%s§a (%dx%dx%d, %d blocks) into your clipboard. Run §e/paste§a to place it.", name, clip.Size.X, clip.Size.Y, clip.Size.Z, clip.Size.Volume())
		}
		return
...
func (Paste) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	origin = cube.Pos{origin.X(), origin.Y() - clip.GroundOffset(), origin.Z()}

	p.Messagef("§aPasting §e%s§a (%dx%dx%d, %d blocks) in the background — this happens in small batches to avoid overloading the server, so it may take a bit on a big one. You'll get a message when it's done.", clip.Name, clip.Size.X, clip.Size.Y, clip.Size.Z, clip.Size.Volume())
	clip.PasteAsync(tx, origin, p.Position(), p.XUID())
}

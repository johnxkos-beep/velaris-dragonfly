...
func (Paste) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	skipped := clip.PasteAt(tx, origin)
	if skipped > 0 {
		p.Messagef("§aPasted §e%s§a (%dx%dx%d, %d blocks, §c%d skipped (furnaces/chests/etc — see notes)§a) at your position.", clip.Name, clip.Size.X, clip.Size.Y, clip.Size.Z, clip.Size.Volume(), skipped)
	} else {

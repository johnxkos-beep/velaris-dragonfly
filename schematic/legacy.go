...
//
// This decodes into a generic map[string]any rather than a typed struct.
// A typed struct (Blocks []byte, then Blocks []int8) failed both times
// with the identical error at the identical byte offset — same tag,
// same position, just a different destination type in the message —
// which means the destination type wasn't the actual problem; something
// about struct-field decoding specifically was. Decoding into a map
// sidesteps that: there's no destination type to mismatch, so whatever
// native Go type the library actually produces for a byte array is read
// back out via toByteSlice below, which accepts either []byte or []int8.
func ReadMcEditSchematic(name string) (*Clipboard, int, error) {
	...
	if err := nbt.NewDecoderWithEncoding(gz, nbt.BigEndian).Decode(&raw); err != nil {
		return nil, 0, fmt.Errorf("couldn't parse .schematic NBT (not a valid MCEdit schematic?): %w", err)
	}

	width, ok1 := toInt(raw["Width"])
	height, ok2 := toInt(raw["Height"])
	length, ok3 := toInt(raw["Length"])
	if !ok1 || !ok2 || !ok3 {
		return nil, 0, errors.New(".schematic is missing Width/Height/Length — not a valid MCEdit schematic")
	}
	...
	blocks, okB := toByteSlice(raw["Blocks"])
	data, okD := toByteSlice(raw["Data"])
	if !okB || !okD {
		return nil, 0, fmt.Errorf(".schematic Blocks/Data tags are an unexpected NBT type (got %T / %T) — not a valid MCEdit schematic", raw["Blocks"], raw["Data"])
	}
	if len(blocks) < size.Volume() || len(data) < size.Volume() {
...
// toInt accepts any of the numeric types an NBT decoder might produce
// for a short/int/long tag (int8/16/32/64, their unsigned counterparts,
// or plain int) and widens it to int.
...
// toByteSlice accepts whichever native Go shape the NBT decoder actually
// produces for a TAG_Byte_Array. This turned out to matter more than
// expected: the error this replaced ("got [550550]uint8") shows the
// decoder returns a fixed-size ARRAY, not a slice — []byte/[]int8 can
// never match an array type since the length is part of the type itself.
// Using reflect here sidesteps needing to know the exact shape (array or
// slice, length N, signed or unsigned byte) up front.

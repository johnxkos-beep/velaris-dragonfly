...
//
// Blocks/Data are []int8, not []byte: gophertunnel's nbt package
// represents TAG_Byte_Array as signed bytes and rejects []byte/[]uint8
// as a type mismatch (this was caught by an in-game runtime error —
// "cannot unmarshal TAG_ByteArray into []uint8" — after the code built
// fine, so the struct tags themselves were right, just this one field
// type). Block IDs/metadata are unsigned 0-255 conceptually, so they're
// re-widened via uint8(...) when read below.
type mcEditSchematic struct {
	Width  int16  `nbt:"Width"`
	Height int16  `nbt:"Height"`
	Length int16  `nbt:"Length"`
	Blocks []int8 `nbt:"Blocks"`
	Data   []int8 `nbt:"Data"`
}
...
func ReadMcEditSchematic(name string) (*Clipboard, int, error) {
	...
	for y := 0; y < size.Y; y++ {
		for z := 0; z < size.Z; z++ {
			for x := 0; x < size.X; x++ {
				blockID := int(uint8(s.Blocks[i]))
				meta := int(uint8(s.Data[i])) & 0xF
				i++

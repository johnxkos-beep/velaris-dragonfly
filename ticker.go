...
func (t *ticker) tickZone(tx *world.Tx, name string, now time.Time) {
	...
	case 1:
		...
		key := holder.Name()
		state.progress[key]++
		holder.SendActionBarMessage("§aCapturing " + name + ": " + strconv.Itoa(state.progress[key]) + "/" + strconv.Itoa(CaptureSeconds) + "s")
	...
	default:
		if len(inside) > 1 {
			for _, p := range inside {
				p.SendActionBarMessage("§c" + name + " is contested!")
			}
...
//
// UNVERIFIED item names: item.Diamond{}, item.GoldenApple{},
// item.NetheriteHelmet{}/NetheriteChestplate{}/NetheriteLeggings{}/
// NetheriteBoots{} are my best-confidence guesses at this Dragonfly
// version's exported struct names, following the exact same naming
// pattern already confirmed working elsewhere in this repo for other
// vanilla items (item.IronIngot{}, item.GoldIngot{}, item.NetheriteScrap{}
// in players/autosmelt.go). If any of these don't exist, `go build` will
// name the real ones and it's a one-line swap per item — nothing else in
// this function depends on the exact names.

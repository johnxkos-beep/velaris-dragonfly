...
// colorCodeByIndex looks up the §-code for a color by its position in
// Colors/ColorNames(). form.Dropdown.Value() returns the selected
// option's index (int), not its label — confirmed by a real build error
// against this exact line, which is what this function fixes. Falls back
// to white on an out-of-range index, which shouldn't happen since the
// dropdown only ever offers len(Colors) options.
func colorCodeByIndex(i int) string {
	if i < 0 || i >= len(Colors) {
		return "§f"
	}

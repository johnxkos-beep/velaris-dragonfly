...
//
// UNVERIFIED: form.Dropdown's zero-arg accessor is assumed to be
// Value() string (the selected option's label), matching the pattern of
// every other element in this project's one custom form (knockback/form.go
// uses Input.Value() string and Toggle.Value() bool) and of dragonfly's
// StepSlider. Nothing in this codebase uses a Dropdown yet to confirm
// against, so if `go build` disagrees, it'll say so on this one line and
// it's a quick fix.
// ---------------------------------------------------------------------

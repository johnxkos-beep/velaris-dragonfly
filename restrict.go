...
// Dragonfly doesn't expose server/block/colour at that path), so this
// uses a coal block instead — also solid black, and, like the pvp
// package's obsidian marker, a plain zero-field block that doesn't depend
// on any colour/variant API that might drift between Dragonfly versions.
func MarkerBlock() block.CoalBlock { return block.CoalBlock{} }
...
func isMarkerBlock(b world.Block) bool {
	...
	return name == "minecraft:coal_block"
}

...
func (r *Runtime) RegisterPlayer(ctx context.Context, p *player.Player) (*Handler, error) {
	...
	// If PlayerJoin itself errors (e.g. a plugin's join listener hits a
	// PHP fatal — like the known BoolGameRule gap), don't abandon the
	// whole session over it. The player object may well already exist
	// PHP-side by the time that happened, and even if it doesn't,
	// leaving the bridge disconnected for this player's entire session
	// (no chat, no commands, nothing) over one failed startup call is
	// far worse than just logging it and continuing.

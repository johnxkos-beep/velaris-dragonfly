...
// converts it to the slot format PHP expects. Empty slots are skipped
// entirely (PHP treats "no slot" the same as "air").
func inventorySlotsFor(p *player.Player) []pmmpcompat.InventorySlot {
	stacks := p.Inventory().All()
	slots := make([]pmmpcompat.InventorySlot, 0, len(stacks))

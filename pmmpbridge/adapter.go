...
var (
	...
	ErrNotInTransaction = errors.New("pmmpcompat dragonfly adapter: player not in a live transaction")
)
...
func DefaultItemMapper(item pmmpcompat.InventoryItem) (dfitem.Stack, error) {
	...
	if !ok {
		// Somewhere upstream in the PHP compat layer, custom item ids
		// (e.g. "bey:midas_sword") sometimes arrive with an extra,
		// incorrect "minecraft:" prefix tacked on ("minecraft:bey:
		// midas_sword"). Rather than track down exactly which PHP call
		// path does that, just tolerate it here: if stripping a
		// leading "minecraft:" leaves something that's still
		// namespaced (i.e. has its own colon — a real custom id, not a
		// genuine vanilla one), retry the lookup with that stripped.
		if trimmed, ok2 := strings.CutPrefix(item.TypeID, "minecraft:"); ok2 && strings.Contains(trimmed, ":") {
			it, ok = world.ItemByName(trimmed, 0)
		}
	}
	if !ok {
...
func (t PlayerTarget) withLiveTransaction(ctx context.Context, f func(*player.Player) error) error {
	...
	// Previously gated on playerTxLive(t.player) — that turned out to be
	// wrong: by the time a slow round trip to PHP (e.g. building a form)
	// finishes, the tx captured at the start of the originating handler
	// is no longer considered "live" even though the player object
	// itself is still perfectly valid to call methods on directly.
	// Dragonfly's *player.Player methods handle their own safety; they
	// don't require the caller to still be inside the same transaction
	// that was active when the triggering event first fired.
	return f(t.player)
}

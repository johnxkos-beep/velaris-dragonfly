...
func (t PlayerTarget) withLiveTransaction(ctx context.Context, f func(*player.Player) error) error {
	...
	if !playerTxLive(t.player) {
		// Round 1 only applies actions while already inside a live
		// transaction — which covers everything join/quit/chat/command
		// events need. Actions fired from outside a transaction (e.g. a
		// PHP scheduled task during Tick) are skipped rather than
		// guessing at an off-thread scheduling API that doesn't match
		// this Dragonfly version. Revisit once actually needed.

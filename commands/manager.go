...
// Delete closes a loaded world and permanently deletes its save data from
// disk. Only worlds this Manager opened (via Open/Load/OpenAll) can be
// deleted — worlds added with Register/RegisterWorld aren't owned by this
// Manager (it doesn't know their on-disk location or lifecycle), so Delete
// refuses those and returns an error instead of silently doing nothing or,
// worse, deleting the wrong thing.
//
// Delete does not evacuate players first — closing the world hands that to
// Dragonfly's own World.Close(). Move players out before deleting if you
// want a clean transition instead of relying on that fallback.
func (m *Manager) Delete(name string) error {
	...
	if !ok {
		m.mu.Unlock()
		return fmt.Errorf("%w: %s", ErrWorldNotFound, clean)
	}
	if !ent.owned {
		m.mu.Unlock()
		return fmt.Errorf("world %q is registered, not owned by this manager — remove it at its source instead", clean)
	}
	...
	if ent.world != nil {
		if err := ent.world.Close(); err != nil {
			return fmt.Errorf("close world %q: %w", clean, err)
		}
	}
	if ent.path != "" {
		if err := os.RemoveAll(ent.path); err != nil {
			return fmt.Errorf("delete world %q files: %w", clean, err)
		}
	}
	return nil
}

...
func (h *PlayerHandler) HandleBlockPlace(ctx *player.Context, pos cube.Pos, b world.Block) {
	...
	if msg, ok, completed := koth.Cfg.OnBlockPlace(h.p.XUID(), pos, b); ok {
		h.p.Message(msg)
		if completed != nil {
			// Spawn at h.p.Position(), NOT completed.SpawnPosition() (the
			// zone's own center) — the center can land in a chunk this
			// tx doesn't already have loaded/locked, and Dragonfly's
			// deferred handling of that case is what crashed the server
			// ("use of transaction after transaction finishes"). The
			// player's own position is always safe: it's guaranteed to
			// be inside whatever this tx already has locked, since
			// that's exactly what this event handler is running for.
			koth.SpawnZoneTicker(h.p.Tx(), h.p.Position(), completed.Corner1)
		}

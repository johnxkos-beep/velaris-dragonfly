...
// package (forced on inside PvP zones, otherwise both players need
// /pvp on) before applying the configured knockback force/height and
// enforcing the attack cooldown — see the knockback package, ported from
...
func (h *PlayerHandler) HandleAttackEntity(ctx *player.Context, e world.Entity, force, height *float64, critical *bool) {
	if victim, ok := e.(*player.Player); ok {
		at := cube.PosFromVec3(h.p.Position())
		if !pvp.Cfg.CombatAllowed(h.p.XUID(), victim.XUID(), at) {
			ctx.Cancel()
			h.p.Message("§cPvP is off here — run /pvp on, or fight inside a PvP zone.")
			return
		}
	}
...
//
// Also forwards every placement to pvp.Cfg.OnBlockPlace, which is a
// no-op unless the player has an active /pvp block corner selection — see
// the pvp package.

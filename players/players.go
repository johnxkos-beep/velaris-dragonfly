...
// HandleMove enforces restrict zones: a move into one is cancelled and
// bounced back out for anyone who isn't an op — see the restrict package.
func (h *PlayerHandler) HandleMove(ctx *player.Context, newPos mgl64.Vec3, _ cube.Rotation) {
	restrict.Cfg.HandleMove(ctx, h.p, newPos)
}

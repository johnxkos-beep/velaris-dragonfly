...
// and also checks pos against every live PvP zone corner, restrict zone
// marker, and KOTH zone corner — breaking any of those deletes it (see
// the pvp, restrict, and koth packages). Breaking a non-corner block
// inside a named KOTH zone's protection box (see koth.IsProtected) is
// cancelled outright for non-ops.
func (h *PlayerHandler) HandleBlockBreak(ctx *player.Context, pos cube.Pos, drops *[]item.Stack, xp *int) {
	if msg, ok := pvp.Cfg.OnBlockBreak(pos); ok {
		h.p.Message(msg)
	}
	if msg, ok := restrict.Cfg.OnBlockBreak(pos); ok {
		h.p.Message(msg)
	}
	if msg, ok := koth.Cfg.OnBlockBreak(pos); ok {
		h.p.Message(msg)
	} else if !state.Ops.IsOp(h.p.XUID()) && !koth.Cfg.IsZoneCorner(pos) && koth.Cfg.IsProtected(pos.X(), pos.Z()) {
		ctx.Cancel()

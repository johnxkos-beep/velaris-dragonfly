...
// but — this is the fix for two real bugs reported after the first
// version shipped — only actually SENDS the title update if the owner is
// CURRENTLY HOLDING the specific weapon this ticker is tracking.
//
// Bedrock's title/subtitle system is a single global on-screen element,
// not scoped to a hotbar slot or item — the first version of this file
// sent updates unconditionally, which meant: (1) switching to an empty
// hand (or any other item) still showed the bar, since nothing ever told
// it to stop, and (2) with two legendaries on cooldown at once, both
// tickers fought over that same single title element, so whichever one's
// tick landed last "won" and made it look like using one weapon put the
// OTHER on cooldown too. Gating the send on "is this the item currently
// in your hand" fixes both: only the tracked weapon's own ticker ever
// paints anything while it's the one actually held, and switching away
// lets the short-lived title (200ms stay, 0 fade) simply expire on its
// own within one update interval — no explicit "hide" packet needed.
// Switching back to that weapon shows the correct, still-accurate
// percentage, since the underlying timer (st.StartedAt/st.Total) keeps
// counting in the background the whole time regardless of what's
// displayed.
func (e *hudBar) Tick(tx *world.Tx, current int64) {
	...
	if !ok || w.def.ID != st.WeaponID {
		return // not holding the tracked weapon right now — stay silent
	}

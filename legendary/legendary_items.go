...
// ITSELF — a real bug from the previous pass: Release() was originally
// added directly to Weapon, the single shared struct used for all 8
// legendary items. Go satisfies interfaces at the TYPE level, not per
...
// item.Releasable, not just Eagle Eye Bow. That almost certainly made
// Dragonfly treat every legendary weapon as a hold-and-release item like a
// bow instead of an immediate-use item, which is why a real report after
// that shipped was "no abilities work at all anymore, not just Eagle Eye
// Bow." BowWeapon embeds Weapon (inheriting all its other methods
// automatically) and is the ONLY type with Release() — and it's only ever
// constructed for "bey:eagle_eye_bow" specifically (see NewWeaponStack and
// Init below), so only that one weapon satisfies item.Releasable now.
//
// UNVERIFIED SIGNATURE: item.Releasable's exact method wasn't confirmed
// against a live build (couldn't fetch the source far enough to see it
// directly) — "Release(tx, u, ctx, dur time.Duration) bool" is a
// reasoned guess following the same shape as the already-confirmed-working
// item.Usable's Use(tx, u, ctx) bool, plus the duration Dragonfly must
// track internally for "how long was this held" (confirmed real via
// Player.UsingItem's doc comment: "using it over a longer duration such as
// when using a bow"). If `go build` disagrees, paste the error — same
// one-signature-fix pattern as every other interface guess in this file.
type BowWeapon struct{ Weapon }

func (w BowWeapon) Release(tx *world.Tx, u item.User, ctx *item.UseContext, dur time.Duration) bool {
	p, ok := u.(*player.Player)
	if !ok {
		return false
	}
	return OnRelease(tx, p, dur)
}

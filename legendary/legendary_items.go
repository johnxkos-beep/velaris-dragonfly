...
// actual logic; every other weapon ignores this (only Eagle Eye Bow reacts
// to it) since draw-and-release is specific to bows.
//
...
func (w Weapon) Release(tx *world.Tx, u item.User, ctx *item.UseContext, dur time.Duration) bool {
	p, ok := u.(*player.Player)
	if !ok {
		return false
	}

...
//
//   - Mjolnir's/Poseidon Trident's "throw" is an INSTANT hitscan strike (a
//     look-ray to the nearest target within range) — not a physical
//     projectile entity that flies out and travels over several ticks.
//     Eagle Eye Bow, however, now fires a REAL vanilla-style arrow entity
//     (see shootEagleEyeBow) via Dragonfly's own built-in arrow
//     constructor — real flight, real collision, real ammo consumption
//     from your inventory, no more instant-hit/no-target-message hack.
...
func (s *playerAbilityState) startCooldown(tx *world.Tx, p *player.Player, w Weapon, weaponID string, ticks int) {
	s.mu.Lock()
...
func applyCooldownUI(tx *world.Tx, p *player.Player, w Weapon, ticks int) {
	dur := time.Duration(ticks) * 50 * time.Millisecond
	p.SetCooldown(w, dur)
	StartCooldownBar(tx, p, w.def.ID, dur)
}
...
//
// NOTE: no longer used by Eagle Eye Bow — see shootEagleEyeBow below,
// which fires a real vanilla-style arrow entity instead of doing an
// instant hitscan+message. Still used by Mjolnir/Poseidon Trident.
...
// shootEagleEyeBow fires a REAL vanilla-style arrow entity (Dragonfly's
// own built-in arrow physics/collision/damage, via entity.DefaultRegistry
// — not a custom hitscan) and consumes one real arrow from the player's
// inventory, same as an actual bow. Requires having an arrow; if you don't,
// nothing fires and no cooldown starts (matches real bow behavior — you
// can't waste the swing on empty ammo). Replaces the old
// findLookTarget/strikeRanged instant-hit approach entirely, which is why
// there's no more "found no target in range" message here — a real arrow
// just flies and hits (or doesn't) like any other arrow, no need to
// announce it.
//
// CONFIRMED (not a guess this time — a real build error told me exactly
// what Arrow's signature actually is, and I looked up the real
// world.ArrowSpawnConfig struct to match it): Arrow takes
// (world.EntitySpawnOpts, world.ArrowSpawnConfig), not a flat list of
// arguments like the first version of this file assumed. Struct fields
// used below (Damage, Owner, ObtainArrowOnPickup) are the real field
// names from github.com/df-mc/dragonfly/server/world/entity.go.
//
// Tip is set to potion.Potion{} rather than left nil — leaving it nil
// caused a real crash last round ("interface conversion: interface {} is
// nil, not potion.Potion"): Dragonfly's arrow code does an unchecked type
// assertion tip.(potion.Potion) internally, and a bare nil doesn't
// satisfy that, panics every time regardless of anything else being
// right. potion.Potion{} is a real, correctly-typed (if cosmetically
// "first" in whatever potion.Potion's zero value represents) value, so
// the type assertion succeeds — confirmed reasoning, not guessed, even
// though the exact "which potion is the zero value" detail isn't
// independently verified (purely cosmetic if it's ever slightly off).
//
// Also fixes a real bug from that same crash: the panic happened AFTER
// the player's arrow had already been taken, meaning every failed shot
// silently ate one of your arrows for nothing. Fixed by reordering
// instead of adding refund logic: this now only PEEKS for an arrow
// (hasArrow) up front, and only actually removes it (takeArrow) after the
// entity has successfully spawned — so a failure here can never cost you
// ammo you didn't get a shot for.
func shootEagleEyeBow(tx *world.Tx, p *player.Player, w Weapon, s *playerAbilityState) {
	if !hasArrow(p) {
...
func shadowBladeCloak(tx *world.Tx, p *player.Player, w Weapon, s *playerAbilityState) {
...
func excaliburShield(tx *world.Tx, p *player.Player, w Weapon, s *playerAbilityState) {
	s.mu.Lock()
...
func dragonKatanaDash(tx *world.Tx, p *player.Player, w Weapon, s *playerAbilityState) {
	s.mu.Lock()

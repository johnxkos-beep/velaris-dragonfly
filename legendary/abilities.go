...
// announce it.
//
// UNVERIFIED: entity.DefaultRegistry.Config().Arrow(...) is Dragonfly's
// own confirmed-real constructor for a vanilla arrow entity (confirmed via
// its documented signature), but the exact parameter meanings for
// critical/disallowPickup/obtainArrowOnPickup/punchLevel/tip are my best
// reading of their names, not independently tested — worst case here is
// cosmetic (e.g. the arrow behaves slightly differently than intended on
// pickup), not a crash.
func shootEagleEyeBow(tx *world.Tx, p *player.Player, w Weapon, s *playerAbilityState) {
	...
	handle := entity.DefaultRegistry.Config().Arrow(
		...
		AttackPoints("bey:eagle_eye_bow"),
		p,     // owner
		false, // critical
		false, // disallowPickup
		true,  // obtainArrowOnPickup
		0,     // punchLevel
		nil,   // tip (no potion effect)
	)
	tx.AddEntity(handle)

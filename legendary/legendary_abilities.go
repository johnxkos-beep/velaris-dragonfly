...
//
//   - Mjolnir/Poseidon Trident's "throw" and Eagle Eye Bow's "shoot" are an
//     INSTANT hitscan strike (a look-ray to the nearest target within
//     range) — not a physical projectile entity that flies out and travels
//     over several ticks. That's still true after this pass, but HOW
//     they're triggered changed: they no longer go through
//     PlayerHandler.HandleItemUse at all (that handler doesn't receive a
//     *world.Tx in this Dragonfly version — confirmed by a real crash log
//     showing it panic on every single use, not just at disconnect).
//     Instead, Weapon in items.go implements item.Usable directly
//     (Use(tx, u, ctx) — the same interface bosses/demonking/spawnegg.go's
//     UsableOnBlock already proves works in this exact codebase for
//     UseOnBlock), which Dragonfly hands a legitimate Tx to as a real
//     parameter. See OnUse below for the 3 weapons that need it; everything
//     self-only (Shadow Blade, Excalibur, Dragon Katana, Trident's
//     sneak-riptide) still goes through OnItemUse/HandleItemUse exactly
...
func (s *playerAbilityState) startCooldown(ctx *item.UseContext, weaponID string, ticks int) {
	s.mu.Lock()
	s.cooldownReady[weaponID] = time.Now().Add(time.Duration(ticks) * 50 * time.Millisecond)
	s.mu.Unlock()
	applyCooldownUI(ctx, ticks)
}

// applyCooldownUI puts weaponID's icon under the sweeping gray "on
// cooldown" overlay on the hotbar — purely cosmetic, doesn't affect
// whether the ability actually works (that's startCooldown's
// cooldownReady map above, which was already correct and enforced before
// this existed).
//
// UNVERIFIED, GENUINELY NEW TERRITORY: nothing else in this codebase has
// ever set a client-visible item cooldown before — knockback's cooldowns
// (knockback/kb.go, knockback/projectile.go) are server-side timestamp
// checks only, no UI, so there's no proven pattern here to copy the way
// there was for Tick(tx) or item.Usable. "ctx.SetCooldown(duration)" is my
// best guess at item.UseContext's method for this (Dragonfly's ender
// pearl uses a fixed-length version of exactly this swipe animation, so
// something in this shape should exist). Isolated to this one function on
// purpose: if `go build` says item.UseContext has no method SetCooldown,
// this is the only place that needs to change — paste the error and
// I'll adjust the one line.
func applyCooldownUI(ctx *item.UseContext, ticks int) {
	if ctx == nil {
		return
	}
	ctx.SetCooldown(time.Duration(ticks) * 50 * time.Millisecond)
}
...
func OnUse(tx *world.Tx, p *player.Player, ctx *item.UseContext) bool {
	defer func() {
		if r := recover(); r != nil {
	...
	case "bey:mjolnir":
		...
		s.startCooldown(ctx, "bey:mjolnir", 900)
	case "bey:poseidon_trident":
		if p.Sneaking() {
			dir := direction(p)
			p.SetVelocity(mgl64.Vec3{dir.X() * 8, 1.2, dir.Z() * 8})
			s.startCooldown(ctx, "bey:poseidon_trident", 400)
			break
		}
	...
	case "bey:eagle_eye_bow":
		...
		s.startCooldown(ctx, "bey:eagle_eye_bow", 40)
	case "bey:shadow_blade":
		shadowBladeCloak(p, s, ctx)
	case "bey:excalibur":
		excaliburShield(p, s, ctx)
	case "bey:dragon_katana":
		dragonKatanaDash(p, s, ctx)
	}
	return true
}
...
func shadowBladeCloak(p *player.Player, s *playerAbilityState, ctx *item.UseContext) {
	...
	s.startCooldown(ctx, "bey:shadow_blade", 1200)
}

// excaliburShield: Resistance 255 + a 5-hit shield charge counter, whichever
// runs out first (10s / 200 ticks either way).
func excaliburShield(p *player.Player, s *playerAbilityState, ctx *item.UseContext) {
	s.mu.Lock()
	...
	p.AddEffect(effect.New(effect.Resistance, 255, 10*time.Second))
	s.startCooldown(ctx, "bey:excalibur", 1200)
}
...
func dragonKatanaDash(p *player.Player, s *playerAbilityState, ctx *item.UseContext) {
	s.mu.Lock()
	...
	s.startCooldown(ctx, "bey:dragon_katana", 200)
}

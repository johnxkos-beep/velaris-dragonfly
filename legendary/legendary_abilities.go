...
// OnItemUse triggers a legendary weapon's SELF-ONLY active ability (Shadow
// Blade's cloak, Excalibur's shield, Dragon Katana's dash, Poseidon
// Trident's sneak-riptide) — the ones that only ever touch the acting
// player's own state and never needed a *world.Tx. Should be called from
// PlayerHandler.HandleItemUse.
//
// Mjolnir, Eagle Eye Bow, and Poseidon Trident's actual throw are handled
// by Weapon.Use in items.go instead (item.Usable's Use(tx, ...) method) —
// PlayerHandler.HandleItemUse doesn't receive a *world.Tx at all in this
// Dragonfly version, confirmed by a real crash (panicked on every single
// use, not just at disconnect, meaning fetching Tx() there is invalid
// full stop) — while item.Usable's Use method is handed a legitimate one
// directly by Dragonfly itself, the same way bosses/demonking/spawnegg.go's
// UsableOnBlock implementation already does in this exact codebase. That's
// what makes real target-scanning safe again for those 3 weapons.
func OnItemUse(ctx *player.Context, p *player.Player) {
	...
	id := w.def.ID
	if id == "bey:mjolnir" || id == "bey:eagle_eye_bow" {
		return // handled by Weapon.Use instead — see doc comment above
	}
	...
	switch id {
	case "bey:poseidon_trident":
		// Only the sneak-riptide branch here — the actual throw (not
		// sneaking) is handled by Weapon.Use instead, since it needs a
		// real target scan. If the player isn't sneaking, do nothing and
		// leave it (and the cooldown) to Weapon.Use.
		if p.Sneaking() {
			dir := direction(p)
...
// OnUse handles Mjolnir, Eagle Eye Bow, and Poseidon Trident's throw (the
// non-sneaking branch) — the 3 ranged abilities that need a real *world.Tx
// to scan for and strike a target. Call this from Weapon.Use in items.go.
// Returns true if an ability actually fired (matches item.Usable's Use
// convention of reporting whether the item did something).
func OnUse(tx *world.Tx, p *player.Player) bool {
	...
	id := w.def.ID
	if id != "bey:mjolnir" && id != "bey:eagle_eye_bow" && id != "bey:poseidon_trident" {
		return false
	}
	if id == "bey:poseidon_trident" && p.Sneaking() {
		return false // riptide branch, handled by OnItemUse instead
	}
...
// stand-in — see the file doc comment on lightning) at pos.

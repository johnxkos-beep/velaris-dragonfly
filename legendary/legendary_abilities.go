...
// names from github.com/df-mc/dragonfly/server/world/entity.go; the
// fields left unset (Critical, DisablePickup, PunchLevel, PiercingLevel,
// Tip) just take their zero values (false/0/nil), which are the sane
// defaults for a plain arrow shot.
...
// consumeArrow removes one "minecraft:arrow" from p's inventory, returning
// false (and taking nothing) if they have none. Same slot-scanning pattern
// as HasIngredients/Craft in manager.go.
func consumeArrow(p *player.Player) bool {
	...
	for slot := 0; slot < inv.Size(); slot++ {
		...
		if err := inv.SetItem(slot, st.Grow(-1)); err != nil {
			return false
		}
		return true
	}

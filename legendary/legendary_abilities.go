...
func (s *playerAbilityState) startCooldown(p *player.Player, w Weapon, weaponID string, ticks int) {
	s.mu.Lock()
	s.cooldownReady[weaponID] = time.Now().Add(time.Duration(ticks) * 50 * time.Millisecond)
	s.mu.Unlock()
	applyCooldownUI(p, w, ticks)
}

// applyCooldownUI puts w's icon under the sweeping gray "on cooldown"
// overlay on the hotbar — purely cosmetic, doesn't affect whether the
...
func applyCooldownUI(p *player.Player, w Weapon, ticks int) {
	p.SetCooldown(w, time.Duration(ticks)*50*time.Millisecond)
}
...
func OnUse(tx *world.Tx, p *player.Player) bool {
	...
	case "bey:mjolnir":
		...
		s.startCooldown(p, w, "bey:mjolnir", 900)
	case "bey:poseidon_trident":
		if p.Sneaking() {
			dir := direction(p)
			p.SetVelocity(mgl64.Vec3{dir.X() * 8, 1.2, dir.Z() * 8})
			s.startCooldown(p, w, "bey:poseidon_trident", 400)
			break
		}
	...
	case "bey:eagle_eye_bow":
		...
		s.startCooldown(p, w, "bey:eagle_eye_bow", 40)
	case "bey:shadow_blade":
		shadowBladeCloak(p, w, s)
	case "bey:excalibur":
		excaliburShield(p, w, s)
	case "bey:dragon_katana":
		dragonKatanaDash(p, w, s)
	}
	return true
}
...
func shadowBladeCloak(p *player.Player, w Weapon, s *playerAbilityState) {
...
func excaliburShield(p *player.Player, w Weapon, s *playerAbilityState) {
	...
	p.AddEffect(effect.New(effect.Resistance, 255, 10*time.Second))
	s.startCooldown(p, w, "bey:excalibur", 1200)
}
...
func dragonKatanaDash(p *player.Player, w Weapon, s *playerAbilityState) {
	s.mu.Lock()
	...
	s.startCooldown(p, w, "bey:dragon_katana", 200)
}

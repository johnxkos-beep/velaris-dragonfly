...
//     with a REAL draw-and-release charge mechanic (see startEagleDraw and
//     OnRelease) via Dragonfly's own built-in arrow constructor — real
//     flight, real collision, real ammo consumption from your inventory,
//     no more instant-hit/no-target-message hack.
...
type playerAbilityState struct {
	...
	eagleDrawing bool // true from startEagleDraw until OnRelease fires or aborts
}
...
//
// NOTE: no longer used by Eagle Eye Bow — see startEagleDraw/OnRelease
...
const minDrawTicks = 4     // matches the add-on's own "if (timer < 4) return" minimum
const maxDrawTicks = 20    // ~1s — matches the add-on's POWER curve maxing out around here
const freezeMaxFall = 0.05 // near-zero downward speed cap while drawing and airborne, matches the add-on's own "maxFallSpeed" anti-fall-while-drawing effect

// startEagleDraw begins Eagle Eye Bow's real charge — matches the add-on's
// own pull-timer system: right-click starts the draw (no shot yet), and
// releasing (OnRelease, via Weapon.Release/item.Releasable) is what
// actually fires, with power scaling by how long you held it. Also spawns
// the ticking entity that applies the add-on's "freeze in the air while
// drawing" effect (see eagleDrawTicker below).
func startEagleDraw(tx *world.Tx, p *player.Player, s *playerAbilityState) {
	if !hasArrow(p) {
		p.Message("§cYou need an arrow to use the Eagle Eye Bow.")
		return
	}
	// DIAGNOSTIC — added to pin down a real report that Eagle Eye Bow
	// "does nothing at all" after the Use/Release split. If this message
	// never shows up when you right-click, that proves Weapon.Use isn't
	// firing for this item at all (a deeper problem with how Dragonfly is
	// recognizing BowWeapon). If it DOES show up, the weapon is drawing
	// correctly and the real issue is what happens on release instead —
	// see the matching diagnostic in OnRelease below. Remove both once
	// this is confirmed working.
	p.Message("§7[debug] Draw started.")
	s.mu.Lock()
	s.eagleDrawing = true
	s.mu.Unlock()
	spawnEagleDrawTicker(tx, p)
}

// OnRelease fires Eagle Eye Bow if the draw was held long enough. Called
// from Weapon.Release (item.Releasable) with however long the player held
// the draw. Power scales with draw time exactly like the add-on's own
// curve: 0 below a minimum draw, up to a 2.5x-equivalent bonus around a
// full ~1s draw.
func OnRelease(tx *world.Tx, p *player.Player, dur time.Duration) bool {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("[legendary] recovered panic in OnRelease for %s: %v", p.Name(), r)
		}
	}()
	...
	s := stateFor(p.XUID())
	s.mu.Lock()
	s.eagleDrawing = false
	s.mu.Unlock()

	ticks := int(dur / (50 * time.Millisecond))
	// DIAGNOSTIC — same purpose as startEagleDraw's, remove once confirmed
	// working. Shows exactly what OnRelease saw: confirms Release() is
	// firing at all, and exactly how many ticks Dragonfly reported the
	// draw lasted (minDrawTicks is 4 — a normal quick tap-and-release,
	// same reflex used for every other weapon in this pack, will
	// legitimately measure well under that, and silently fired nothing
	// before this message existed).
	p.Message(fmt.Sprintf("§7[debug] Released after %d ticks (need %d+).", ticks, minDrawTicks))
	if ticks < minDrawTicks {
		return false // released too early — matches the add-on exactly, no shot fired
	}
	if !hasArrow(p) {
		p.Message("§cYou need an arrow to use the Eagle Eye Bow.")
		return false
	...
	handle := entity.DefaultRegistry.Config().Arrow(
		...
		world.ArrowSpawnConfig{
			Damage:              AttackPoints("bey:eagle_eye_bow") + power*2,
			Owner:               p,
...
// this once you're sure it's actually needed (see OnRelease) — unlike
// hasArrow, this mutates the inventory.

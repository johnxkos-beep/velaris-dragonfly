...
// and 1 block up, plays the add-on's "mob.endermen.portal" sound, and
// spends 50 charge. Below 50 charge, tells the player they don't have
// enough and does nothing else (no cooldown spent) — matches the add-on's
// own onUse exactly, including the exact sound and message.
func dragonKatanaDash(p *player.Player, s *playerAbilityState, tx *world.Tx) {
	...
	p.Teleport(dest)
	// UNVERIFIED: "sound.EndermanTeleport{}" is a guess at Dragonfly's name
	// for this sound (matches the add-on's own "mob.endermen.portal"). If
	// it doesn't exist under this name, any other teleport/enderman sound
	// in the sound package is a fine substitute — cosmetic only.
	tx.PlaySound(p.Position(), sound.EndermanTeleport{})

...
func buildDefs() map[string]*Def {
	raw := []*Def{
		...
		},
		{
			...
			Lore: []string{
				"§6Damage: §rDeals the same base damage as a Diamond Sword",
				"§6Special: §rBuilds §lcharge§r from kills — §l+50§r per player, §l+1§r per mob",
				"§6Interact: §rAt §l50+ charge§r, dash forward and consume 50 charge",
				"§6Strategy: §rChain kills to keep dashing back into the fight",
			},
			Recipe: []Ingredient{
...
func NewWeaponStack(id string) (item.Stack, bool) {
	...
	var w world.Item = Weapon{def: d}
	if id == "bey:eagle_eye_bow" {
		w = BowWeapon{Weapon{def: d}}
	}
	s := item.NewStack(w, 1)
...
// eagleeye.go) via the legendaryItem interface below, so that code can
// work with either Weapon or BowWeapon without needing to know which
// concrete type it's holding.
func (w Weapon) WeaponDef() *Def { return w.def }

// legendaryItem is satisfied by both Weapon and BowWeapon (BowWeapon gets
// it for free via embedding Weapon). Every place outside this file that
// used to do `held.Item().(Weapon)` now does `held.Item().(legendaryItem)`
// instead — asserting to the concrete Weapon type specifically would fail
// for Eagle Eye Bow now that it's actually a BowWeapon under the hood (see
// the doc comment on BowWeapon above for why that split exists).
...
// Release implements item.Releasable — Eagle Eye Bow's real charge-and-fire
// mechanic (matches the add-on's own draw timer exactly: minimum 4-tick
// draw to fire at all, power scales up over the draw, capped at a 2.5x
// multiplier around a ~1s full draw). See OnRelease in abilities.go for the
// actual logic.
//
// THIS METHOD LIVES ON A SEPARATE TYPE (BowWeapon, below), NOT ON Weapon
// ITSELF — a real bug from an earlier pass: Release() was originally added
// directly to Weapon, the single shared struct used for all 8 legendary
// items. Go satisfies interfaces at the TYPE level, not per
// instance/field-value — so the moment Weapon had a Release method, EVERY
// legendary weapon (Mjolnir, Shadow Blade, all of them) satisfied
// item.Releasable, not just Eagle Eye Bow, which broke every weapon at
// once. BowWeapon embeds Weapon (inheriting all its other methods
// automatically) and is the ONLY type with Release()/Requirements() — and
// it's only ever constructed for "bey:eagle_eye_bow" specifically (see
// NewWeaponStack and Init below), so only that one weapon satisfies
// item.Releasable now.
//
// CONFIRMED against real Dragonfly source this time (not a guess): a
// prior version of this method had the parameter order wrong —
// Release(tx, u, ctx, dur) — which silently meant BowWeapon never actually
// satisfied item.Releasable at all (Go's interface matching is strict
// about parameter order/types), so Release() never fired even once,
// confirmed by a real test where holding the bow produced zero feedback.
// The real signature, read directly from Dragonfly's own player.go
// (specifically the call site `i.Item().(item.Releasable).Release(p,
// p.tx, useCtx, dur)`), is user-first, no bool return:
type BowWeapon struct{ Weapon }

func (w BowWeapon) Release(user item.User, tx *world.Tx, ctx *item.UseContext, dur time.Duration) {
...
// Requirements implements the other half of item.Releasable — also
// confirmed real from source (Player.canRelease reads this to check ammo
// before even allowing the draw to start), which an earlier pass missed
// entirely: only having Release() still wouldn't have satisfied the full
// interface even with the right signature. Returns one arrow, matching a
// real vanilla bow's own requirement.
//
// UNVERIFIED: "item.Arrow{}" as the concrete Go type for a vanilla arrow
// item is a reasoned guess (matches this codebase's established vanilla
// item naming, e.g. item.DiamondSword elsewhere) but wasn't independently
// confirmed. If wrong, it's an isolated one-line fix here — the actual
// ammo consumption (takeArrow in abilities.go, which scans by the real
// "minecraft:arrow" identifier string) doesn't depend on this being
// right; worst case here is Dragonfly's own pre-draw ammo gate not
// working, not a functional break in firing itself.
func (w BowWeapon) Requirements() []item.Stack {
	return []item.Stack{item.NewStack(item.Arrow{}, 1)}
}
...
func Init(claimsPath string) error {
	for _, id := range Order {
		d := Defs[id]
		if id == "bey:eagle_eye_bow" {
			world.RegisterItem(BowWeapon{Weapon{def: d}})
			continue
		}

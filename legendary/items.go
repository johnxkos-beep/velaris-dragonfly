...
func NewWeaponStack(id string) (item.Stack, bool) {
	if id == "bey:midas_sword" {
		// PILOT: block-item render path (see block_render.go) instead of the
		// generic flat-icon Weapon below.
		return NewMidasSwordStack(), true
	}
	d, ok := Defs[id]
	if !ok {
...
// HandEquipped is an optional world.CustomItem interface. Without it,
// Dragonfly's generated resource pack leaves "minecraft:hand_equipped"
// unset/false, and the Bedrock client renders the item held flat and
// centered like a block — which is the "just holding an item" look you're
// seeing. Setting this true switches to the raised/angled tool-hold pose
// vanilla swords and axes use, so the sword-shaped icon actually reads as a
// weapon in hand instead of a flat card.
func (w Weapon) HandEquipped() bool { return true }
...
func Init(claimsPath string) error {
	for _, id := range Order {
		if id == "bey:midas_sword" {
			// PILOT: registers as both item and block in one call, since
			// MidasSwordItem implements both interfaces (see block_render.go).
			// Both calls are required — RegisterItem alone leaves the block
			// registry without an entry for its hash, which panics the
			// moment a player holding/spawning near it triggers a block
			// runtime ID lookup.
			world.RegisterItem(MidasSwordItem{})
			world.RegisterBlock(MidasSwordItem{})
			continue
		}
		d := Defs[id]

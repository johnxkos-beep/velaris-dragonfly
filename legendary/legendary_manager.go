...
// Reset un-claims a single weapon (op-only, used by /legendary's "Reset All
// Legendaries" admin option — see form.go). Returns false if it wasn't
// claimed to begin with.
func (m *Manager) Reset(weaponID string) bool {
	m.mu.Lock()
	defer m.mu.Unlock()
	if _, ok := m.claims[weaponID]; !ok {
		return false
	}
	delete(m.claims, weaponID)
	m.save()
	return true
}

// ResetAll un-claims every legendary weapon, matching the original PHP
// plugin's "/menu reset all" admin command.

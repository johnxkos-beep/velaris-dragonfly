...
// centre — not wherever that world's saved spawn happens to be. Every
// portal built this way is tracked (see registry.go) so /endportaldelete
// can remove them all again later.
type Command struct{}
...
func (Command) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	triggerMin, triggerMax, footprintMin, footprintMax := BuildEntryPortal(tx, base)
	handle := SpawnSentinel(tx, triggerMin, triggerMax, Destination{World: EndDestinationName, LandX: islandCentreX, LandZ: islandCentreZ})
	record(tx.World(), footprintMin, footprintMax, handle)
...
// DeleteCommand is /endportaldelete — op-only, named after the same
// pattern as the existing /worlddelete command. Removes every entry portal
// built with /endportal IN THE CURRENT WORLD (wherever the op is standing
// when they run it) — clearing the frame/portal blocks and despawning
// their sentinels. Portals built in a different world than the one you're
// currently in aren't touched; run this from each world you built one in.
// This does NOT touch the exit portal the Ender Dragon opens on death
// (bosses/enderdragon) — that's tied to the boss fight, not something this
// tracks.
type DeleteCommand struct{}

func (DeleteCommand) Allow(src cmd.Source) bool { return state.IsOpSource(src) }

func (DeleteCommand) Run(src cmd.Source, output *cmd.Output, tx *world.Tx) {
	n := DeleteAllIn(tx)
	if n == 0 {
		output.Print("No End portals tracked in this world.")
		return
	}
	output.Printf("Removed %d End portal(s).", n)
}

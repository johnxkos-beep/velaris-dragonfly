...
// once they arrive — LandX/LandZ, not the destination's saved spawn. If
// LandRadius is >0, the actual landing spot is randomized within that many
// blocks of LandX/LandZ each time (still always ground-scanned by
// landSafely for a genuinely safe surface point — randomizing never risks
// landing someone underground, it just picks a different column to scan).
...
// portal to work — same reasoning as restrict's ensureEnforcer spawning
// next to the calling player instead of at a fixed/possibly-unloaded spot.
// Returns the entity's handle so callers can record it for later removal
// (see registry.go's DeleteAllIn).
func SpawnSentinel(tx *world.Tx, min, max cube.Pos, dest Destination) *world.EntityHandle {
	centre := mgl64.Vec3{
...
func (s *sentinel) travel(tx *world.Tx, p *player.Player) {
	dest := s.st.Dest
	x, z := dest.LandX, dest.LandZ
	if dest.LandRadius > 0 {
		x += rand.Intn(2*dest.LandRadius+1) - dest.LandRadius
		z += rand.Intn(2*dest.LandRadius+1) - dest.LandRadius
	}
	err := state.Worlds.TravelPlayerTx(tx, p, dest.World, withAfter(func(tx2 *world.Tx, p2 *player.Player) error {
		landSafely(tx2, p2, x, z)
		return nil
	}))
	if err != nil {

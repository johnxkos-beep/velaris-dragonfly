...
//
// Returns the inner 3x3 portal-block box (in block coordinates, at
// base.Y()-1) for passing straight to SpawnSentinel.
func BuildEntryPortal(tx *world.Tx, base cube.Pos) (min, max cube.Pos) {
	frameY := base.Y() - 1
	...
	min = cube.Pos{base.X() - 1, frameY, base.Z() - 1}
	max = cube.Pos{base.X() + 1, frameY, base.Z() + 1}
	return min, max
}

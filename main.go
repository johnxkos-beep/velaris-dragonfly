...
func main() {
	...
	// One-off world-save cleanup. Runs strictly before conf.New() below
	// opens the server's own handle on the world folder, since
	// mcdb/leveldb only allows one open handle on a world at a time.
	//
	// To use: set cleanEntitiesEnabled to true below, push, reinstall the
	// server so it rebuilds, and start it — watch the console for
	// "cleaning world save entities" / "world entity cleanup done". Then
	// set it back to false and push again, so it doesn't redo the same
	// box (harmless, but pointless) on every future boot.
	const (
		cleanEntitiesEnabled = false
		cleanEntitiesX       = 1400
		cleanEntitiesZ       = -1400
		cleanEntitiesRadius  = 8 // chunks — 8 covers a 272x272 block box
	)
	if cleanEntitiesEnabled {
		log.Info("cleaning world save entities", "x", cleanEntitiesX, "z", cleanEntitiesZ, "radiusChunks", cleanEntitiesRadius)
		n, err := worldmaint.CleanEntities("world", world.Overworld, cleanEntitiesX, cleanEntitiesZ, cleanEntitiesRadius, conf.Entities, log)
		if err != nil {
			log.Error("world entity cleanup failed", "error", err.Error())
			os.Exit(1)
		}
		log.Info("world entity cleanup done", "entitiesRemoved", n)
	}

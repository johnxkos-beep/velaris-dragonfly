...
import (
	...
	"velaris-dragonfly/worldmaint"
)

// envInt reads an integer from the named environment variable, falling
// back to def if it's unset or not a valid integer. Used for the
// Pterodactyl-tunable CLEAN_* startup variables.
...
func main() {
	...
	// Strip leftover vanilla-Bedrock mob entities (squid/cow/creeper/etc.)
	// baked into the custom map's original export. Dragonfly has no Go
	// implementation for them, so they silently fail to load every time —
	// logging an "unknown entity type" error each time, which doubles up
	// whenever two players converge on the same chunk and is what's been
	// causing the tick lag behind the timeout kicks near each other. This
	// must run before conf.New() opens the world below (mcdb only allows
	// one open handle on a world folder at a time) — see
	// worldmaint/cleanentities.go's own doc comment for the full story.
	//
	// UNVERIFIED: userConf.World.Folder is my best-documented read of
	// where Dragonfly's default config points the world save directory
	// (falls back to "world" if left unset, which is the standard
	// Dragonfly default) — not confirmed against your exact version from
	// this environment. If the field name is wrong, go build will say so
	// immediately.
	//
	// CLEAN_RADIUS_CHUNKS/CLEAN_CENTER_X/CLEAN_CENTER_Z let you tune the
	// swept area via Pterodactyl's Startup > Variables without touching
	// code. Defaults sweep a 96x96 chunk box (1536 blocks each way from
	// 0,0) which should comfortably cover most map exports — if your map
	// is larger than that, or centered somewhere other than 0,0, bump
	// these. This only needs to run once per world save; after the first
	// clean, re-running it is a fast no-op scan.
	worldDir := userConf.World.Folder
	if worldDir == "" {
		worldDir = "world"
	}
	cleanRadius := envInt("CLEAN_RADIUS_CHUNKS", 96)
	cleanCenterX := envInt("CLEAN_CENTER_X", 0)
	cleanCenterZ := envInt("CLEAN_CENTER_Z", 0)
	for _, dim := range []world.Dimension{world.Overworld, world.Nether} {
		removed, err := worldmaint.CleanEntities(worldDir, dim, cleanCenterX, cleanCenterZ, cleanRadius, demonking.EntityRegistry(), log)
		if err != nil {
			log.Error("world entity cleanup failed", "dimension", fmt.Sprint(dim), "error", err.Error())
			continue
		}
		if removed > 0 {
			log.Info("world entity cleanup removed leftover entities", "dimension", fmt.Sprint(dim), "removed", removed)
		}
	}

	// conf.Generator intentionally left unset now that a custom map is
	// loaded (worldgen/ removed). Dragonfly falls back to its own default
	// generator (a flat superflat world) for any chunk not already saved
	// in your uploaded map — i.e. anything past its edges. If you want to
	// hard-stop players from ever reaching that flat area rather than
	// just generating into it, set a world border instead.

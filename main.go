...
import (
	...
	"velaris-dragonfly/legendaryweapons"
...
func (h *combinedHandler) HandleBlockPlace(ctx *player.Context, pos cube.Pos, b world.Block) {
	// players.PlayerHandler's own version cancels placement of the
	// internal "*_display_block" pseudo-blocks — that has to run first
	// and keep working exactly as before. The bridge forwarding is
	// additional, for PHP-side listeners like HopliteLegendary's
	// ZoneListener/BuilderListener (pvp/restrict/builder marker
	// blocks).
	h.PlayerHandler.HandleBlockPlace(ctx, pos, b)
	if h.bridge != nil {
		h.bridge.HandleBlockPlace(ctx, pos, b)
	}
}

func (h *combinedHandler) HandleBlockBreak(ctx *player.Context, pos cube.Pos, drops *[]item.Stack, xp *int) {
	// Same idea — players.PlayerHandler's autosmelt logic runs first
	// and keeps working as before, bridge forwarding is additional (for
	// PHP-side block-protection/zone-removal checks).
	h.PlayerHandler.HandleBlockBreak(ctx, pos, drops, xp)
	if h.bridge != nil {
		h.bridge.HandleBlockBreak(ctx, pos, drops, xp)
	}
}

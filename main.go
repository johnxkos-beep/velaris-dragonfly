...
func registerCommands() {
	...
	// EasyEdit schem loading port: //load lists/loads a schematic file into
	// memory, //paste places it. Registered with a leading "/" in the name
	// itself so the in-game "//load" / "//paste" convention works.
	cmd.Register(cmd.New("/load", "Lists or loads a schematic file.", nil, schematics.Load{}))
	cmd.Register(cmd.New("/paste", "Pastes your currently loaded schematic.", nil, schematics.Paste{}))

	// MultiWorld port: create/delete/list/info/rename/load/unload/teleport
	// between independently-generated worlds.
	cmd.Register(cmd.New("mw", "Manage multiple worlds.", []string{"multiworld"},
		multiworld.MWCreate{}, multiworld.MWDelete{}, multiworld.MWList{},
		multiworld.MWInfo{}, multiworld.MWRename{}, multiworld.MWLoad{},
		multiworld.MWUnload{}, multiworld.MWTeleport{}))
}
...
func main() {
	...
	// AutoBuildPack defaults to false — without this, Dragonfly never
	// builds/sends a resource pack at all, no matter how many custom items
	// are registered. This was the actual missing piece: it's not
	// automatic just because world.RegisterItem was called.
	userConf.Resources.AutoBuildPack = true

	// Folder is where the generated pack gets written. This was left as
	// Go's zero value ("") before, which may be why AutoBuildPack silently
	// had no effect — the official default template always sets this to
	// "resources". Setting it explicitly so there's an actual place on
	// disk for the pack to land.
	userConf.Resources.Folder = "resources"

	// Required forces the client to actually go through the download
	// prompt (and kicks them with a clear message if they back out),
	// instead of silently reusing a cached "no" from a previous optional
	// prompt for this server — which is the most likely reason you're
	// still not seeing any prompt at all after AutoBuildPack alone.
	userConf.Resources.Required = true

	// Custom items must be registered with world.RegisterItem before
	// conf.New() below — that's when Dragonfly bakes its auto-generated
	// resource pack from whatever's currently registered. Registering
	// afterward (as this used to do) still works at the Go/inventory level
	// but the client is never offered a pack containing the new items, so
	// they show up with no icon.
	if err := legendary.Init("legendary_claims.json"); err != nil {
		log.Error("failed to load legendary weapons", "error", err.Error())
		os.Exit(1)
	}

	// Schematics folder replaces PocketMine's plugin_data/EasyEdit — upload
	// .schem files here over SFTP, same as before.
	if err := schematics.Init("schematics"); err != nil {
		log.Error("failed to set up schematics folder", "error", err.Error())
		os.Exit(1)
	}

	// MultiWorld registry — tracks every world ever created via /mw,
	// independent of which are currently loaded.
	if err := multiworld.InitRegistry("multiworld.json"); err != nil {
		log.Error("failed to load multiworld registry", "error", err.Error())
		os.Exit(1)
	}
	multiworld.SetDeps(multiworld.Deps{
		Log: log,

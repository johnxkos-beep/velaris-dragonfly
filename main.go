...
// kick, and ban all work from the Pterodactyl console box.
...
func runConsole() {
	...
	for scanner.Scan() {
		...
		case "op":
			...
			target, ok := findOnline(targetName)
			if !ok {
		...
		case "kick":
			if len(args) < 1 {
				fmt.Println("Usage: kick <player> [reason]  (use \"quotes\" around names with spaces)")
				continue
			}
			targetName, reasonArgs := splitQuotedName(args)
			target, ok := findOnline(targetName)
			if !ok {
				fmt.Printf("Player '%s' is not online.\n", targetName)
				continue
		...
		case "ban":
			if len(args) < 1 {
				fmt.Println("Usage: ban <player> [reason]  (use \"quotes\" around names with spaces)")
				continue
			}
			targetName, reasonArgs := splitQuotedName(args)
			target, ok := findOnline(targetName)
			if !ok {
				fmt.Printf("Player '%s' is not online (only online players can be banned right now).\n", targetName)
				continue
			}
			reason := "Banned by an operator."
			if len(reasonArgs) > 0 {
			...
			if err := globalBans.Ban(target.XUID(), reason); err != nil {
				fmt.Println("Failed to save ban:", err)
				continue
			}
			target.Disconnect(reason)
		...
		default:
			fmt.Println("Unknown console command:", name, "(available: op, deop, kick, ban)")
		}
	}
}

// splitQuotedName handles console input like `kick "Velaris Founder" being
// rude` — if the first argument starts with a `"`, it treats everything up
// to the matching closing `"` as one name (spaces included), and returns
// the remaining words separately as the reason. If there's no leading
// quote, it falls back to treating just the first word as the name.
func splitQuotedName(args []string) (name string, rest []string) {
	...
	if end == -1 {
		// No closing quote found; treat the whole thing as the name.
		return strings.TrimSuffix(joined, `"`), nil
	}
	name = joined[:end]
	remainder := strings.TrimSpace(joined[end+1:])
	if remainder == "" {
		return name, nil
	}
	return name, strings.Fields(remainder)
}

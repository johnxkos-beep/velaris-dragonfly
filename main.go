...
func main() {
	// Debug level temporarily, to surface recovered panics / errors that
	// Dragonfly logs at Debug (e.g. chunk decode failures) which were
	// previously silently dropped at the default Info level.
	//
	// Also mirrored to velaris.log on disk (via io.MultiWriter) so it can
	// be opened/downloaded from the Pterodactyl Files tab and searched
	// (e.g. for "panic" or "level=ERROR") instead of scrolling the
	// console on mobile.
	logFile, ferr := os.OpenFile("velaris.log", os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if ferr != nil {
		// Print to stdout directly (logger isn't built yet) so this is
		// visible in console instead of failing silently.
		fmt.Println("WARNING: could not open velaris.log for writing:", ferr.Error())
		logFile = nil
	}

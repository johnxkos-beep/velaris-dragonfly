...
func main() {
	...
	scoreboardStop := make(chan struct{})
	go scoreboardMgr.Run(scoreboardStop, func() []*player.Player {
		var online []*player.Player
		for p := range state.Server.Players(nil) {
			online = append(online, p)
		}
		return online
	})
	defer close(scoreboardStop)
	...
	for p := range srv.Accept() {
		...
		onlineCount := 0
		for range state.Server.Players(nil) {
			onlineCount++
		}

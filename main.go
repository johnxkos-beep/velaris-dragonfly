...
func main() {
	...
	if cleanEntitiesEnabled {
		...
		n, err := worldmaint.CleanEntities("world", world.Overworld, cleanEntitiesX, cleanEntitiesZ, cleanEntitiesRadius, log)
		if err != nil {
			log.Error("world entity cleanup failed", "error", err.Error())
			os.Exit(1)
		}
		log.Info("world entity cleanup done", "chunksProcessed", n)
	}

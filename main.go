...
func registerCommands() {
	...
	cmd.Register(cmd.New("border", "View or change the world border size.", nil, border.Command{}))
	cmd.Register(cmd.New("koth", "Manage King of the Hill zones.", nil, koth.Block{}, koth.Name{}, koth.Activate{}, koth.Time{}, koth.List{}))
	...
	cmd.Register(cmd.New("worldcreate", "Creates or loads a new DFWorlds destination world.", nil, commands.WorldCreate{}, commands.WorldCreateTyped{}))
	...
	cmd.Register(cmd.New("end", "Travels you to the End.", nil, commands.End{}))
	cmd.Register(cmd.New("spawnenderdragon", "Builds the arena and spawns the Ender Dragon at your position.", nil, enderdragon.SpawnCommand{}))
	cmd.Register(cmd.New("endportal", "Opens a real End portal at your position.", nil, endportal.Command{}))
	cmd.Register(cmd.New("endportaldelete", "Removes every End portal built with /endportal in the current world.", nil, endportal.DeleteCommand{}))
	cmd.Register(cmd.New("cooldown", "Update the chat cooldown", nil, chatcooldown.Command{}))
	cmd.Register(cmd.New("count", "Open the countdown setup form", nil, countdown.Command{}))
	cmd.Register(cmd.New("save", "Saves a region to a schematic file.", nil, schematic.Save{}))
	cmd.Register(cmd.New("load", "Lists schematics, or loads one into your clipboard.", nil, schematic.Load{}))
	cmd.Register(cmd.New("paste", "Pastes your loaded schematic at your position.", nil, schematic.Paste{}))
	cmd.Register(cmd.New("news", "Broadcasts a server-wide announcement.", nil, news.Command{}))
	cmd.Register(cmd.New("track", "Names or checks distance to a shared track point.", nil, track.Point{}, track.Off{}, track.Usage{}, track.Track{}))
}
...
func main() {
	...
	// Caps how many chunks around each player get loaded/simulated.
	// Dragonfly's default (32) is tuned for a handful of players; at 100
	// concurrent, view distance is normally the single biggest per-tick
	// CPU and per-player bandwidth cost on a Bedrock server, since it's
	// multiplied by every online player independently. 8 trades some
	// render distance for meaningfully lower load at this player count —
	// raise it later if headroom allows.
	// UNVERIFIED FIELD NAME: this is my best-documented read of
	// Dragonfly's UserConfig (no network access from this environment to
	// confirm against your exact v0.11.1 struct). If `go build` reports
	// this field doesn't exist, tell me the exact field it suggests
	// instead — one-line fix.
	userConf.Players.MaximumChunkRadius = 8
	...
	// Forces every connecting client onto ONLY this server's resource
	// pack(s), ignoring anything they've enabled locally (Lunar-style HUD
	// packs, etc). Without this, a player's own pack can stack alongside
	// yours and win the conflict on shared UI files like hud_screen.json
	// — which is exactly what the custom crosshair depends on. This is
	// almost certainly why your friend saw no crosshair on Lunar but you
	// did with no pack at all.
	// UNVERIFIED FIELD NAME: same caveat as Resources.Folder above — no
	// network access here to confirm against your exact Dragonfly
	// version. If `go build` disagrees, tell me what it suggests.
	userConf.Resources.Required = true
	...
	entityTypes = append(entityTypes, border.EntityTypes()...)
	entityTypes = append(entityTypes, koth.EntityTypes()...)
	...
	entityTypes = append(entityTypes, countdown.EntityTypes()...)
	entityTypes = append(entityTypes, enderdragon.EntityTypes()...)
	entityTypes = append(entityTypes, endportal.EntityTypes()...)
	entityTypes = append(entityTypes, news.EntityTypes()...)
	entityTypes = append(entityTypes, track.EntityTypes()...)
	...
	border.Cfg, err = border.Load("border.json")
	if err != nil {
		log.Error("failed to load border config", "error", err.Error())
		os.Exit(1)
	}
	koth.Cfg, err = koth.Load("koth.json")
	if err != nil {
		log.Error("failed to load koth config", "error", err.Error())
		os.Exit(1)
	}
	chatcooldown.Cfg, err = chatcooldown.Load("chatcooldown.json")
	if err != nil {
		log.Error("failed to load chat cooldown config", "error", err.Error())
		os.Exit(1)
	}
	...
	track.Cfg, err = track.Load("track_points.json")
	if err != nil {
		log.Error("failed to load track points", "error", err.Error())
		os.Exit(1)
	}
	...
	for p := range srv.Accept() {
		...
		countdown.EnsureTicker(p.Tx(), p.Position())
		news.EnsureTicker(p.Tx(), p.Position())
		track.EnsureTicker(p.Tx(), p.Position())
		border.EnsureTicker(p.Tx(), p.Position())
		koth.EnsureAllZoneTickers(p.Tx(), p.Position())
		mobs.EnsureSpawner(p.Tx(), p.Position())
		mobs.EnsureHostileSpawner(p.Tx(), p.Position())
		// Same join-time kickoff pattern as the tickers above — without this,
		// the crosshair ticker never starts until someone uses a legendary
		// weapon ability, and since the resource pack fully replaces vanilla
		// crosshair rendering, nobody sees ANY crosshair until then. See
		// legendary/crosshair.go's doc comment for the full history.
		legendary.EnsureCrosshair(p.Tx())

...
// OnlineCount returns the number of currently tracked online players.
// Safe to call from anywhere, including synchronously inside the
// srv.Accept() loop right after TrackJoin — unlike state.Server.Players(nil),
// which must never be called reentrantly from inside Accept()'s own loop
// body (that's a second, independent iteration over the server's player
// registry while the first one — Accept() — hasn't finished processing
// this player's join yet; an earlier version of main.go did exactly that
// and it self-deadlocked the join, with no log line and no error).
